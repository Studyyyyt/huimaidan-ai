...
import { EXCLUDE_LOGIN_PATH_LIST, FORCE_LOGIN_PATH_LIST, isNeedLoginMode, LOGIN_PAGE, LOGIN_PAGE_ENABLE_IN_MP, MER_ADMIN_PATHS } from './config'
...
export const navigateToInterceptor = {
  ...
  invoke({ url, query }: { url: string, query?: Record<string, string> }) {
    ...
    if (FORCE_LOGIN_PATH_LIST.includes(path)) {
      tokenStore.updateNowTime()
      if (!tokenStore.hasLogin) {
        uni.navigateTo({ url: `${LOGIN_PAGE}?redirect=${encodeURIComponent(url)}` })
        return false
      }
    }

    // 小程序里面使用平台自带的登录时，除强制登录业务页外，不走下面的通用逻辑
    if (isMp && !LOGIN_PAGE_ENABLE_IN_MP) {
      return true // 明确表示允许路由继续执行
    }

    // 商户管理页面使用独立的 mer-token 认证，不走用户 token 校验
    if (MER_ADMIN_PATHS.includes(path)) {
      const merTokenStore = useMerTokenStore()
      merTokenStore.updateNowTime()
      FG_LOG_ENABLE && console.log('商户管理页面, merTokenStore.hasLogin:', merTokenStore.hasLogin)

      // 商户登录页直接放行
      if (path === '/pages/admin/login') {
        return true
      }

      // 其他商户管理页面需要商户已登录
      if (merTokenStore.hasLogin) {
        return true // 商户已登录，允许访问
      }
      // 商户未登录，跳转到商户登录页

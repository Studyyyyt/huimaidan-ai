...
<template>
  <view class="flex flex-col items-center justify-center">
    ...
    <template v-if="item.iconType === 'unocss' || item.iconType === 'iconfont'">
      <view
        :class="[(tabbarStore.curIdx === index && item.iconActive) ? item.iconActive : item.icon, isBulge ? 'text-80px' : 'text-20px']"
        :style="isBulge ? {} : { width: '24px', height: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center' }"
      />
    </template>

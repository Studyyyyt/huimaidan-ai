...
import { tabbarList as _tabbarList, adminTabbarList, selectedTabbarStrategy, TABBAR_STRATEGY_MAP } from './config'
...
// 管理员专用 tabbar 列表
const adminTabbarListItems = reactive<CustomTabBarItem[]>(
  adminTabbarList.map(item => ({
    ...item,
...
const isAdminLogin = computed(() => {
  ...
  return merTokenStore.hasLogin
})

const tabbarList = computed(() => {
  // 如果是管理员登录状态，显示管理员专用 tabbar
  if (isAdminLogin.value) {
    return adminTabbarListItems
  }

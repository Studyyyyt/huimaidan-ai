...
import { selectWeixinBaseUrl } from './env-base-url'
...
export function getEnvBaseUrl() {
  ...
  if (isMpWeixin) {
    ...
    baseUrl = selectWeixinBaseUrl({
      defaultBaseUrl: baseUrl,
      envVersion,
      weixinDevelopBaseUrl: import.meta.env.VITE_SERVER_BASEURL_WEIXIN_DEVELOP,
      weixinTrialBaseUrl: import.meta.env.VITE_SERVER_BASEURL_WEIXIN_TRIAL,
      weixinReleaseBaseUrl: import.meta.env.VITE_SERVER_BASEURL_WEIXIN_RELEASE,
    })
  }
...
export function normalizeMediaUrl(url?: string | null) {
  ...
  if (!value || value.startsWith('http://') || value.startsWith('https://') || value.startsWith('data:') || value.startsWith('blob:') || value.startsWith('/static/')) {
    return value
  }
  if (value.startsWith('//')) {
    return `https:${value}`

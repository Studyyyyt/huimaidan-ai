...
import { openMenuPath } from '@/utils/menuPath';
...
export default {
  ...
  methods: {
    ...
    onColumnsAsideMenuClick(v) {
      let { path, redirect } = v;
      if (v.children) {
        openMenuPath(this, findFirstNonNullChildren(v.children).path);
      } else {
        openMenuPath(this, path);
      }
      // 一个路由设置自动收起菜单
    ...
    setFilterRoutes() {
      ...
      this.bus.$emit('oneCatName', resData.item&&resData.item[0].title);
      this.onColumnsAsideDown(resData.item&&resData.item[0].k);
      // 刷新时，初始化一个路由设置自动收起菜单

<template>
  ...
  >
    ...
    <div class="layout-navbars-breadcrumb-user-icon" v-if="!isAgent">
      <div class="platformLabel">平台</div>
    </div>
    <ZoneSelector v-else>
      <template #default="{ zoneName }">
        <div class="layout-navbars-breadcrumb-user-icon">
          <div class="platformLabel line1" :title="zoneName">{{ zoneName }}</div>
        </div>
      </template>
    </ZoneSelector>
    <el-dropdown
...
export default {
  ...
  components: { UserNews, Search, ZoneSelector },
  data() {
    return {
      roterPre: roterPre,
      isScreenfull: false,
      ...
      adminName: Cookies.set("AdminName"),
      list: [],
      dealtList: []
    };
  },
            ...
            beforeClose: async (action, instance, done) => {
              if (action === "confirm") {
                ...
                setTimeout(async () => {
                  ...
                  const redirectPath = isAgent ? 'agent-login' : 'login';
                  this.$router.push(
                    `${roterPre}/${redirectPath}?redirect=${this.$route.fullPath}`
                  );
                  done();
                }, 150);
              } else {

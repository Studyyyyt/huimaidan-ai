<template>
  <div class="c_product" v-if="configData">
    ...
    <div class="list-box">
      <draggable class="dragArea list-group" :list="configData.list" group="peoples" handle=".move-icon">
        <div class="item" v-for="(item, index) in configData.list" :key="index" @click="activeBtn(index)">
          ...
          <div class="acea-row" style="padding-bottom: 20px;padding-right: 15px;">
            <div class="move-icon">
          ...
          <div v-if="configData.tabCur == index">
            <div class="conter">
              ...
              <div class="c_row-item">
                <el-col class="c_label" :span="4"> 选择方式 </el-col>
                <el-col class="color-box" :span="19">
                  <el-select size="small" style="width: 100%" v-model="item.tabVal" placeholder="请选择"
                    @change="tabChange">
                    <el-option v-for="(itemn, indexn) in typeList" :value="itemn.activeValue" :key="indexn"
                      :label="itemn.title"></el-option>
                  </el-select>
                </el-col>
              </div>
              <div class="goods-box acea-row" v-if="item.tabVal == 1">
                ...
                <div class="list">
                  <draggable class="dragArea list-group" :list="item.goodsList.list" group="peoples">
                    <div class="items" v-for="(goods, gIndex) in item.goodsList.list" :key="gIndex"
              ...
              <div v-else>
                <div class="c_row-item" v-if="item.tabVal == 6">
                  <el-col class="label" :span="4">品牌名称</el-col>
                  <el-col :span="19" class="slider-box">
                    <el-cascader @change="brandChange" placeholder="请选择品牌" size="mini"
                      v-model="item.brandConfig.brandVal" :options="brandData" :props="props" filterable clearable>
                    </el-cascader>
                  </el-col>
                </div>
                <div class="c_row-item" v-else-if="item.tabVal == 2">
                  <el-col class="label" :span="4">商品分类</el-col>
                  <el-col :span="19" class="slider-box">
                    <el-cascader @change="sliderChange" placeholder="请选择分类" size="small"
                      v-model="item.selectConfig.activeValue" :options="treeSelect" :props="categoryProps" filterable clearable>
                    </el-cascader>
                  </el-col>
                </div>
                <div class="c_row-item" v-else-if="item.tabVal == 3">
                  ...
                  <el-col :span="19" class="slider-box">
                    <el-select multiple size="small" style="width: 100%" v-model="item.goodsLabel.activeValue" placeholder="请选择"
                      @change="tabChange">
                      <el-option v-for="(itemn, indexn) in goodsLabelList" :value="itemn.activeValue" :key="indexn"
                        :label="itemn.title"></el-option>
                    </el-select>
                  </el-col>
                </div>
                <c_select small :configObj="item" configNme="goodsType" v-else-if="item.tabVal == 4" @getConfig="getConfig" />
                <c_select small :configObj="item" configNme="deliveryType" v-else-if="item.tabVal == 5" @getConfig="getConfig" />
                <div class="c_row-item">
                  ...
                  <el-col :span="19" class="slider-box on">
                    <!-- sliderChange -->
                    <el-slider v-model="item.numConfig.val" show-input :show-input-controls="false"
                      @change="radioChange()" :max="100" :min="1" :step="1"></el-slider>
                  </el-col>
                </div>
                <div class="c_row-item">
                  <el-col class="c_label" style="margin-bottom: 15px" :span="4"> 商品排序 </el-col>
                  <el-col class="color-box" :span="19">
          ...
          <div class="delete" @click.stop="bindDelete(index)">
            <i class="iconfont icondel_1" style="font-size: 20px;" />
          </div>
        </div>
      </draggable>
    </div>
    ...
    <!-- 商品标签 -->
    <el-dialog :visible.sync="storeLabelShow" scrollable title="选择商品标签" :closable="true" width="540" :footer-hide="true"
      :mask-closable="false">
      <storeLabelList v-if="storeLabelShow" ref="storeLabel" @activeData="activeStoreData" @close="storeLabelClose">
      </storeLabelList>
    </el-dialog>
    <el-dialog :visible.sync="modals" title="商品列表" class="paymentFooter" width="900">
      <goods-list ref="goodslist" :ischeckbox="true" :isdiy="true" isType @getProductId="getProductId"
...
import c_select from '@/components/mobileConfigRight/c_select.vue';
...
export default {
  ...
  data() {
    return {
      ...
      categoryProps: { multiple: true, checkStrictly: false, emitPath: false },
      defaults: {},
      configData: {},
      itemObj: {},
  ...
  methods: {
    async getGoodsLabel() {
      try {
        ...
        this.goodsLabelList = res.data.list.map(el => {
    ...
    getPic(pc) {
      this.$nextTick(() => {
        this.configData.list[this.configData.tabCur].image = pc.att_dir[0];
        this.modalPic = false;
      });
    },
    getBrandList() {
    ...
    unique(arr) {
      const res = new Map();
      return arr.filter((item) => !res.has(item.product_id) && res.set(item.product_id, 1));
    },
    getProductId(data) {
      this.modals = false;

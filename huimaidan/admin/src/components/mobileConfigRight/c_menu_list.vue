  ...
  <div class="hot_imgs">
    ...
    <div class="list-box">
      <draggable class="dragArea list-group" :list="configData.list" group="peoples" handle=".move-icon">
        <div class="item" v-for="(item, index) in configData.list" :key="index">
          <div class="delect-btn" @click.stop="bindDelete(item, index)" v-if="!configData.isCube && (configData.list.length !== 1 || index !== 0)">
          ...
          <div class="info">
            <div class="info-item" v-for="(infos, key) in item.info" :key="key">
              ...
              <div class="input-box">
                <el-input v-model="infos.value" :placeholder="infos.tips" :maxlength="infos.max" :show-word-limit="!!infos.showLimit">
    ...
    <template v-if="configData.list">
      <div class="add-btn" v-if="configData.list.length < configData.maxList">
        <el-button class="btn" ghost @click="addBox">
...
export default {
  ...
  methods: {
    ...
    getPic(pc) {
      this.$nextTick(() => {
        this.configData.list[this.activeIndex].img = pc.att_dir[0];

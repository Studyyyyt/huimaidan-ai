<template>
  <div class="fillets" v-if="configData">
    ...
    <div class="c_row-item on" v-if="configData.type">
      ...
      <div class="right">
        <el-input
          ...
          :class="index > 1 ? '' : 'on'"
          type="number"
          @change="handleSingleRadiusChange(configData.valList[index])"
    ...
    <div class="c_row-item" v-else>
      ...
      <el-col :span="18">
        <el-slider v-model="configData.val" show-input :show-input-controls="false" :min="configData.min" :max="configData.max || 100"></el-slider>
      </el-col>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    handleSingleRadiusChange(config) {
      const { max, val } = config;
      const valNum = Number(val);
      if (Number.isNaN(valNum) || valNum < 0) {
        config.val = 0;
      } else if (max && valNum > max) {
        config.val = max;
      }
    },
    radioChange(e) {
...
<style>
.el-main .el-radio-button:first-child .el-radio-button__orig-radio:checked+.el-radio-button__inner {

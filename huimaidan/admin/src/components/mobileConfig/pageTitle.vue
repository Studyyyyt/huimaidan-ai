      ...
      <el-col :span="19" class="slider-box">
        ...
        <div class="acea-row row-between row-top color">
          ...
          <el-radio-group v-model="tabVal" size="mini" @input="radioTap">
            <el-radio-button :label="index" v-for="(item, index) in bgTypeList" :key="index">
              <el-tooltip class="item" effect="dark" :content="item.intro" placement="top">
                <span class="iconfont-diy" :class="item.icon"></span>
              </el-tooltip>
            </el-radio-button>
          </el-radio-group>
        </div>
...
export default {
  ...
  methods: {
    ...
    getPic(pc) {
      this.$nextTick(() => {
        this.bgPicUrl = pc.att_dir[0];
        this.modalPic = false;
        this.$store.commit('mobildConfig/UPPICURL', pc.att_dir[0]);
      });
    },

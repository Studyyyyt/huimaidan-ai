...
export default {
  ...
  methods: {
    categoryList() {
      articleListApi()
        .then((res) => {
          let data = [];
          res.data.map((item) => {
            data.push({ title: item.title, activeValue: item.article_category_id.toString() });
          });
    ...
    getConfig(data) {
      return;
      if (!data ||data.name == 'radio') {
        return;
      }
      let val = {
        cid: parseInt(this.configObj.selectConfig.activeValue),
        page: 1,

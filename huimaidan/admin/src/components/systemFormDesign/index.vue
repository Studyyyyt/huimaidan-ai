    ...
    <el-card>
      <div class="diy-wrapper" :style="'height:' + clientHeight + 'px;'">
        ...
        <div class="left">
          <div
            class="wrapper"
            :style="'height:' + (clientHeight - 96) + 'px;'"
            v-if="tabCur == 0"
          >
              ...
              <div class="scrollCon">
                <div style="width: 460px;margin: 0 auto">
                  <div
                    ...
                    ref="imgContainer"
                  >
                    <div class="mConfig-item" style="cursor: not-allowed">
                      <component v-for="(item, indexz) in readonlyList" :key="indexz" :is="item.name" :defaultPageData="item" />
                    </div>
                    <draggable
                      class="dragArea list-group"
                      :list="mConfig"
...
export default {
  ...
  created() {
    ...
    const formData = this.iconfig;
    if (formData && formData.value) {
      this.getDefaultConfig(JSON.parse(JSON.stringify(formData)));
    }
  },
  mounted() {
    this.$nextTick(() => {
  ...
  methods: {
    updateScreenSize() {
      const headerHeight = document.querySelector(`.layout-header`).clientHeight;
      const screenHeight = document.documentElement.clientHeight;
      const availableHeight = screenHeight - headerHeight - 140;
      const H = availableHeight - 100;
    ...
    saveConfig() {
      ...
      this.$nextTick(function() {
        ...
        for (const item of Object.values(val)) {
          if (item.titleConfig && titleSet.has(item.titleConfig.value)) {
            this.loading = false;
            return this.$message.error(`存在重复标题：${item.titleConfig.value}`);
          }
          titleSet.add(item.titleConfig.value);
        }
        Object.assign(this.iconfig, {
    ...
    getDefaultConfig(data) {
      if (data.name) this.storeName = data.name;
      let obj = {};
      let tempARR = [];

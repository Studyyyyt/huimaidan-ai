...
export default {
  ...
  computed: {
    ...
    wrapperStyle() {
      ...
      return {
        marginTop: diyUtil.buildMarginTopOffset({ val: this.mTOP }, this.renderData.offsetYConfig),
        ...
        boxShadow: this.compShadowConfig,
      };
    },
    list() {
      if (this.renderData) {
        const list = this.renderData.selectList.list;
    ...
    queryWhere() {
      ...
      return {
        page: 1,
        limit: parseInt(this.renderData.numConfig.val),
        cid: parseInt(this.renderData.selectConfig.activeValue),
      };
    }
  },
  watch: {
  ...
  methods: {
    getArticleList(query) {
      articleLstApi(query)
        .then((res) => {
          this.renderData.selectList.list = res.data.list;
        })
        .catch((err) => {
    ...
    setConfig(data) {
      ...
      if (data.mbConfig) {
        ...
        this.likeColor = data.likeSuccessColor.color[0].item;
        ...
        this.compShadowConfig = diyUtil.buildShadowStyle(data.compShadowConfig);
      }
    },
  },
};

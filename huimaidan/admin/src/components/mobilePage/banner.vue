<template>
  <div class="mobile-page" :style="bannerWrapStyle">
    <div
    ...
    <div v-else :style="style2BannerStyle">
      <div
        class="banner"
    ...
    <div>
      ...
      >
        ...
        ></div>
        <template v-if="imgSrc.length > 1">
          <div
          class="dot-item"
          :style="{ background: toneConfig ? dotBgColor : '#ddd' }"
          v-for="(item, index) in imgSrc.length - 1"
          :key="index"
          ></div>
        </template>
      </div>
...
export default {
  ...
  computed: {
    ...
    imgShadowStyle() {
      ...
      return {
        boxShadow: diyUtil.buildShadowStyle(this.renderData.imgShadowConfig),
      };
    },
    bannerWrapStyle() {
      if (!this.renderData) return {};
      const { mbConfig, offsetYConfig } = this.renderData;
      return {
        boxShadow: diyUtil.buildShadowStyle(this.renderData.shadowConfig),
        marginTop: diyUtil.buildMarginTopOffset(mbConfig, offsetYConfig)
      };
    },
    style2BannerStyle() {
      return {

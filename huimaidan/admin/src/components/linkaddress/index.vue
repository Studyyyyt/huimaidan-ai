<template>
  <div class="divBox">
    <el-dialog v-if="modals" :visible.sync="modals" title="选择链接" width="860px" :before-close="cancel" append-to-body>
			<div class="table_box">
				<div class="left_box" v-if="defaultType !== 'micro'">
					<el-tree
						:data="categoryData"
						default-expand-all
						node-key="id"
				...
				<div class="right_box" v-if="currenType=='special' || currenType == 'article_category' || currenType=='product_category' || currenType=='points_cate' || currenType=='product' || currenType=='seckill' ||currenType=='presell' || currenType == 'bargain' || currenType == 'combination' || currenType == 'news' || currenType == 'active' || currenType == 'merchant' || currenType == 'micro' || currenType == 'activity_form'">
					<div v-if="currenType=='micro'" class="micro-search-box">
						<span class="micro-search-label">页面名称:</span>
						<el-input size="small" placeholder="请输入页面名称" v-model="formValidate.templateName" @keyup.enter.native="getDiyPageList" class="micro-search-input" />
						<el-button size="small" type="primary" @click="getDiyPageList">搜索</el-button>
					</div>
					<el-form
						ref="formValidate"
						:model="formValidate"
					...
					<el-table
						...
						:load="handleLazyload"
					>
						<el-table-column key="1" label="选择" min-width="50">
							<template slot-scope="scope">
								<span style="width: 1px;height: 1px; display: inline-block;"></span>
								<el-radio v-model="radio" :label="scope.$index"  @change.native="getCurrentRow(scope.row)"><span>&nbsp;</span></el-radio>
							</template>
						</el-table-column>
						<el-table-column key="2" prop="id" label="ID" min-width="50">
						<template slot-scope="scope">
							<span>{{ getRowId(scope.row) }}</span>
						</template>
						</el-table-column>
						<el-table-column v-if="currenType=='special' || currenType=='micro'" key="3" label="页面名称">
							<template #default="{ row }">
								<el-tooltip effect="dark" :content="currenType=='special' ? row.title : row.name" placement="top-start">
									<span class="line1">
										{{ currenType=='special' ? row.title : row.name }}
									</span>
								</el-tooltip>
							</template>
						</el-table-column>
						<el-table-column v-if="currenType=='special'" key="4"  prop="url" label="页面链接" min-width="150" />
						<el-table-column v-if="currenType=='product_category' || currenType=='points_cate' || currenType=='article_category'" key="5"  prop="cate_name" label="分类名称" min-width="60" />
						...
						<el-table-column v-if="currenType=='merchant'" key="11" label="店铺头像" min-width="150">
							<template slot-scope="scope">
								<div class="demo-image__preview">
						...
						<el-table-column v-if="currenType=='merchant'" key="14"  prop="mer_name" label="店铺名称" min-width="150" />
						...
						<el-table-column v-if="currenType=='micro'" key="16"  prop="add_time" label="创建时间" min-width="140" />
						<el-table-column v-if="currenType=='micro'" key="17"  prop="update_time" label="更新时间" min-width="140" />
...
// +----------------------------------------------------------------------
	import { pageCategory, pageLink, getProduct, diyList, merchantDiyList } from "@/api/diy";
	import { categoryListApi } from "@/api/product";
	import { articleLstApi, articleListApi } from '@/api/cms';
    export default {
      name: 'linkaddress',
			props: {
				defaultType: {
      ...
      data () {
				const isStore = (new URLSearchParams(window.location.search)).get('store') === '1';
      return {
        modals: false,
				categoryData:[],
				currenType: this.defaultType,
				type: isStore ? 1 : 0,
				radio: '',
				props: {
					emitPath: false
		...
		watch: {
			modals(val) {
				if (val && this.tableList.length === 0) {
					if (this.defaultType !== 'micro') {
						this.getSort();
					} else {
						this.getDiyPageList();
    ...
    methods: {
			getRowId(row) {
				return row.id || row.product_id || row.group_data_id || row.mer_id || row.article_id || row.store_category_id || row.activity_id || row.article_category_id;
			},
			handleLazyload(row, treeNode, resolve) {
				setTimeout(() => {
      ...
      getSort(){
				...
				articleListApi().then(res => {
					this.tableList = res.data.map(item => {
						return {
							...
							url: `/pages/news_list/index?id=${item.article_category_id}`
						}
					});
			...
			getDiyPageList() {
				...
				this.loading = true;
				const api = this.$route.query.store === "1" ? merchantDiyList : diyList;
				api({
					page: this.formValidate.page,
					limit: this.formValidate.limit,
					name: this.formValidate.templateName
				}).then(res => {
					this.tableList = res.data.list.map(item => {
						return {
							...item,
							id: item.id,
							url: `/pages/small_page/index?id=${item.id}`
						}
					});
					this.total = res.data.count;
					this.loading = false;
        ...
        let id = ''
				...
				if(this.currenType == 'product' || this.currenType == 'seckill' || this.currenType == 'presell' || this.currenType == 'bargain' || this.currenType == 'combination' || this.currenType == 'news'){
					...
					this.getCustomList();
				} else if (this.currenType == 'article_category') {

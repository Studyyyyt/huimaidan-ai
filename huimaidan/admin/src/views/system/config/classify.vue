<template>
  <div class="divBox">
    ...
    <el-card class="mt14">
      ...
      >
        <el-table-column prop="config_classify_id" label="ID" min-width="80" />
        <el-table-column
          prop="classify_name"
          label="配置分类名称"
          min-width="150"
        ...
        <el-table-column
          ...
          min-width="150"
        />
        <el-table-column prop="info" label="配置分类说明" min-width="150" />
        ...
        <el-table-column label="操作" min-width="100" fixed="right">
          <template slot-scope="scope">
            <el-button
              ...
              type="text"
              size="small"
              @click="onConfigList(scope.row.config_classify_id)"
              >配置列表</el-button
            >
            <el-button
              type="text"
              size="small"
              @click="onEdit(scope.row.config_classify_id)"
              >编辑</el-button
            >
            <el-button
              type="text"
              size="small"
              @click="handleDelete(scope.row.config_classify_id, scope.$index)"
              >删除</el-button
            >
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      configClassifyLst(this.status, this.classify_name)
        .then(res => {
    ...
    onchangeIsShow(row) {
      changeConfigClassifyStatus(row.config_classify_id, row.status)
        .then(({ message }) => {
    ...
    onConfigList(config_classify_id) {
      this.$router.push({
        path: this.roterPre + "/config/setting",
        query: {
          config_classify_id: config_classify_id
        }
      });
    }
  }

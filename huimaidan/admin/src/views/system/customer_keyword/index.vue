  ...
  <div class="divBox">
    <el-card>
      ...
      <el-table v-loading="listLoading" :data="tableData.data" size="small">
        <el-table-column prop="service_reply_id" label="ID" min-width="60" />
        <el-table-column prop="keyword" label="关键字" min-width="150" />
        <el-table-column prop="status" label="回复类型" min-width="100">
        ...
        <el-table-column label="操作" min-width="100" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button
              type="text"
              size="small"
              @click="handleDelete(scope.row.service_reply_id, scope.$index)"
              >删除</el-button
            >
          </template>
        </el-table-column>
...
export default {
  ...
  data() {
    ...
    const validateSrc = (rule, value, callback) => {
      if (this.formValidate.type == "2" && this.formValidate.content == "") {
  ...
  methods: {
    onchangeIsShow(row) {
      replyStatusApi(row.service_reply_id, row.status)
        .then(({ message }) => {
    ...
    getList() {
      ...
      replyListApi(this.tableData.page, this.tableData.limit)
        .then(res => {
    ...
    operation() {
      this.$confirm("继续添加吗?", "提示", {
        ...
        .catch(() => {
          setTimeout(() => {
            this.$router.push({
              path: `${roterPre}/systemForm/customer_keyword`
            });
          }, 500);
        });
    },

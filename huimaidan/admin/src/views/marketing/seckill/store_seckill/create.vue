<template>
  <div class="divBox">
    <div class="container_box">
      ...
      <el-card class="box-card box-body mt14" :bordered="false" shadow="never">
        <el-tabs v-model="activeName">
          ...
          <el-tab-pane
            v-if="isEdit || !active_id != '' || isCopy"
            label="秒杀商品"
            name="second"
          ></el-tab-pane>
          <el-tab-pane
            v-if="active_id != '' && !isCopy"
            label="已加商品"
            name="third"
          ></el-tab-pane>
        </el-tabs>
        ...
        >
          <div v-loading="loading">
              ...
              <el-form-item label="活动边框图：">
                <div class="acea-row row-middle">
                  <div
                    ...
                    @click="modalPicTap('2')"
                  >
                    <div v-if="ruleForm.border_pic" class="pictrue">
                      <img :src="ruleForm.border_pic" />
                      <i
                        v-if="!isLook"
                        class="el-icon-error btndel"
                        @click.stop="ruleForm.border_pic = ''"
                      />
                    </div>
            ...
            <template
              v-if="activeName == 'second' && (isEdit || !active_id || isCopy)"
            >
                  ...
                  >
                    ...
                    <el-dropdown-menu slot="dropdown">
                      <el-dropdown-item command="limit"
                        >设置限量</el-dropdown-item
                      >
                      <el-dropdown-item command="price"
                        >设置价格</el-dropdown-item
                      >
              ...
              >
                ...
                >
                  ...
                  >
                    <span v-if="isLook">{{ scope.row.rank }}</span>
                    <el-input-number
                      v-else
            ...
            <template v-if="activeName == 'third' && active_id != ''">
              <el-form size="small" inline>
                ...
                <el-form-item label="商品搜索：">
                  <el-input
                    ...
                    class="selWidth"
                    clearable
                    @keyup.enter.native="getJoinedList(1)"
                  />
                </el-form-item>
                <el-form-item>
                  <el-button
    ...
    <goodsList
      ref="goodsList"
      :pid="ruleForm.product_category_ids"
      :proData="proData"
...
import { categoryListApi, seckillProductLstApi } from "@/api/product";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        seckill_time_ids: [
          { required: true, message: "请选择秒杀场次", trigger: "change" }
        ],
        ...
        merStars: [
          { required: true, message: "请选择店铺星级", trigger: "change" }
        ]
      },
      pickerOptions: {
        disabledDate(time) {
  ...
  computed: {
    //是否是编辑
    isEdit() {
      return this.$route.params.type == "edit" && this.$route.params.id
        ? true
        : false;
    },
    isCopy() {
      return this.$route.params.type == "copy" && this.$route.params.id
        ? true
        : false;
    },
    isLook() {
      return this.$route.params.type == "look" && this.$route.params.id
        ? true
        : false;
    },
    title() {
      return this.$route.params.type == "look" && this.$route.params.id
        ? "查看"
        : this.$route.params.type == "edit" && this.$route.params.id
        ? "编辑"
  ...
  methods: {
    setTagsViewTitle() {
      if (this.$route.params.id) {
        const title = this.$route.params.id ? "编辑秒杀活动" : "添加秒杀活动";
    ...
    modalPicTap(type) {
      ...
      if (!_this.isLook) {
        this.$modalUpload(function(img) {
          if (type == 1) {
            _this.ruleForm.atmosphere_pic = img[0];
          } else {
            _this.ruleForm.border_pic = img[0];
          }
        });
      }
    },
    ...
    changeStatus(row) {
      if (row.children && row.children.length > 0) {
        ...
        if (
          i != -1 &&
          this.proData[i]["children"] &&
          this.proData[i]["children"].length > 0
        ) {
          if (this.proData[i]["children"].every(item => item.is_show == 0)) {
            this.proData[i]["is_show"] = 0;
          } else if (
            this.proData[i]["children"].every(item => item.is_show == 1) ||
            this.proData[i]["children"].some(item => item.is_show === 1)
          ) {
    ...
    // 获取活动相关的商品列表
    getProductList(id) {
      getSelectProductList({ seckill_active_id: id })
        .then(res => {
            ...
            if (type == "limit") {
              ...
              if (data.limit_type == 0) {
                ...
                this.$set(
                  item1,
                  "stock",
                  item1.stock - data.activity_stock > 0
                    ? item1.stock - data.activity_stock
                    : 0
                );
              ...
              if (data.type == 0) {
                ...
                this.$set(
                  item1,
                  "price",
                  item1.price - data.price > 0 ? item1.price - data.price : 0.01
                );
              } else if (data.type == 2) {
              ...
              if (item["children"].every(itm => itm.is_show == 0)) {
                ...
                item["children"].every(itm => itm.is_show == 1) ||
                item["children"].some(itm => itm.is_show === 1)
              ) {
                this.$set(item, "is_show", 1);
              }
            }
    ...
    getSeckillIntervalList() {
      seckillIntervalListApi({ active_id: 0 }).then(res => {
    ...
    getAttrValue(row, isEdit) {
      const _this = this;
      row.map(item => {
        _this.$set(item, "rank", item.rank ? item.rank : 0);
        _this.$set(
          item,
          "mer_name",
        ...
        item.attrValue.map(i => {
          ...
          _this.$set(i, "is_show", item.is_show || 0);
    ...
    filterIsShowOne(arr) {
      return arr.map(item => {
        if (item.attrValue && item.attrValue.length > 0) {
          const filteredChildren = item.attrValue.filter(
            child => child.is_show == 1
          );
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (this.proData.length > 0) {
            ...
            this.ruleForm.product_list = this.filterIsShowOne(arr);
            if (!total && total !== 0 && this.ruleForm.product_list.length > 0)
              return this.$message.warning("商品限量不能为空");
            if (!price && this.ruleForm.product_list.length > 0)
              return this.$message.warning("商品秒杀价格不能为空");
            if (
              total < this.proData.length &&
              this.ruleForm.product_list.length > 0
            )
              return this.$message.warning("商品限量总和不能小于0");
          }
          this.isEdit
            ? seckillAtivityUpdateApi(this.$route.params.id, this.ruleForm)
                .then(res => {
                  this.$message.success(res.message);
                  this.$router.push({
                    path: `${
                      this.roterPre
                    }/marketing/seckill/store_seckill/list`
                  });
                })
                .catch(res => {
                  this.$message.error(res.message);

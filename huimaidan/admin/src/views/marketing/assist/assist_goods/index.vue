  ...
  <div class="divBox">
    ...
    <el-card class="mt14">
      <el-table v-loading="listLoading" :data="tableData.data" size="small">
        ...
        <el-table-column prop="store_name" label="助力商品" min-width="200">
          <template slot-scope="scope">
            <div class="product-info">
              <div class="demo-image__preview">
                <el-image :src="scope.row.product.image" :preview-src-list="[scope.row.product.image]" />
              </div>
              <span>{{ scope.row.product.store_name }}</span>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column  label="助力成功人数/参与人次"  min-width="90">
          <template slot-scope="scope">
...
import {
  ...
  assistProUpdateApi,
  assistStatusApi,
  merSelectListApi, assistProductUpdateApi,
  updatetAssistLabel,
  getProductLabelApi
} from '@/api/product'
...
export default {
  name: "ProductList",
  components: { infoFrom, detailsData, previewBox, WangEditor },
  data() {
    return {
      props: {
        emitPath: false,

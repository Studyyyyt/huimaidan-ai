<template>
  <div class="divBox">
    ...
    <el-card class="mt14">
      <div>
        <router-link
          :to="{ path: `${roterPre}` + '/marketing/integral/addProduct' }"
        >
          <el-button size="small" type="primary">添加积分商品</el-button>
        </router-link>
        <el-button size="small" type="success" @click="quickAdd()" class="ml10"
          >快速添加</el-button
        >
      </div>
      <el-table
          ...
          <template slot-scope="scope">
            <router-link
              :to="{
                path:
                  `${roterPre}` +
                  '/marketing/integral/orderList?id=' +
    ...
    <!--快速添加商品-->
    <goods-list ref="goodsList" @getProduct="getProduct" append-to-body></goods-list>
  </div>
</template>
...
import {
  integralProList,
  integralProDeleteApi,
  integralProductStatusApi
} from "@/api/marketing";
...
export default {
  ...
  methods: {
    ...
    getProduct(row) {
      this.$router.push({
        path: this.roterPre + `/marketing/integral/addProduct/${row.id}?type=1`
      });
    },
    onchangeTime(e) {
      this.timeVal = e;

    ...
    <el-card>
      <el-table v-loading="listLoading" :data="tableData.data" size="small">
        ...
        <el-table-column label="开团团长" min-width="100">
          <template slot-scope="scope">
            <span>{{
              scope.row.initiator && scope.row.initiator.nickname
            }}</span>
          </template>
        </el-table-column>
        <el-table-column label="拼团商品图片" min-width="80">
          <template #default="{ row }">
            <div class="demo-image__preview" v-if="row.productGroup && row.productGroup.product">
              <el-image
                :src=" row.productGroup.product.image"
                :preview-src-list="[row.productGroup.product.image]"
              />
            </div>
          </template>
        </el-table-column>
        <el-table-column label="拼团商品" min-width="150">
          <template #default="{ row }">
            <span v-if="row.productGroup && row.productGroup.product">{{ row.productGroup.product.store_name }}</span>
          </template>
        </el-table-column>

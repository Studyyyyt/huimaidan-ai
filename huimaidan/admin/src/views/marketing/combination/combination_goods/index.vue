  ...
  <div class="divBox">
    ...
    <el-card class="mt14">
      <el-table v-loading="listLoading" :data="tableData.data" size="small">
        ...
        <el-table-column prop="mer_name" label="店铺类别" min-width="80">
          <template slot-scope="scope">
        ...
        <el-table-column
          ...
          min-width="150"
        />
        <el-table-column prop="product.price" label="划线价" min-width="80" />
        <el-table-column prop="price" label="拼团价" min-width="60" />
        <el-table-column label="几人团/成团数量" min-width="80" align="center">
          <template slot-scope="scope">
            <span
              >{{ scope.row.buying_count_num }} /
              {{ scope.row.success_num }}</span
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template slot-scope="scope">
            <span>{{
              scope.row.action_status === 0
                ? "未开始"
                : scope.row.action_status === 1
                ? "正在进行"
                : "已结束"
            }}</span>
          </template>
        ...
        <el-table-column label="操作" min-width="180" fixed="right">
          <template slot-scope="scope">
            <el-button
              type="text"
              size="small"
              @click="onEdit(scope.row.product_group_id)"
              >编辑</el-button
            >
            <el-button
              type="text"
              size="small"
              @click="handlePreview(scope.row.product_group_id)"
              >预览</el-button
            >
            ...
            <el-button
              ...
              type="text"
              size="small"
              @click="toReview(scope.row.product_group_id)"
              >审核</el-button
            >
            <el-button
              type="text"
              size="small"
              @click="goDetail(scope.row.product_group_id)"
              >详情</el-button
            >
            <el-button
              ...
              type="text"
              size="small"
              @click="toOff(scope.row.product_group_id, scope.$index)"
              >强制下架</el-button
            >
          </template>
        </el-table-column>
...
export default {
  name: "ProductList",
  components: { infoFrom, detailsData, WangEditor, previewBox },
  data() {
    return {
      props: {
        emitPath: false
      ...
      tableFrom: {
        ...
        keyword: "",
        active_type: "",
        product_status: this.$route.query.status
  ...
  methods: {
    ...
    submitForm(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          updatetCombinationLabel(this.product_id, this.labelForm).then(
            ({ message }) => {
    ...
    toOff(id) {
      this.$prompt("", "强制下架", {
        ...
        .then(({ value }) => {
          combinationProductStatusApi({ id: id, status: -2, refusal: value })
            .then(res => {
              this.$message({
                type: "success",
                message: "提交成功"
    ...
    handleSubmit(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          combinationProductUpdateApi(this.productId, this.formValidate)
            .then(async res => {
              this.fullscreenLoading = false;

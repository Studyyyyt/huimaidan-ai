        ...
        <div class="head">
          <div class="full">
            ...
            <div class="text">
              <div class="title">
                {{ orgDetail.name }}
                <el-tag size="mini" :style="getOrgTypeStyle()" class="ml10">
                  {{ tagContent }}
                </el-tag>
              </div>
              <div class="acea-row">ID: {{ orgDetail.store_group_id }}</div>
            </div>
          </div>
        </div>
        ...
        <el-tabs type="border-card" v-model="activeTab" class="zone-detail-tabs">
          ...
          <div class="section" v-show="activeTab === TAB_NAME.BASIC_INFO">
            <ul class="list column">
              <li class="item">
                <div>上级分组：</div>
                <div class="value">{{ parentOrgName }}</div>
              </li>
              <li class="item">
                <div>分组名称：</div>
              ...
              <li class="item">
                <div style="width: 5em; text-align: right;">排序：</div>
                <div class="value">{{ orgDetail.sort }}</div>
              </li>
              <li class="item">
                <div>开启状态：</div>
              ...
              <li class="item">
                <div>区域定位：</div>
                <div class="value">{{ orgDetail.positioning_status ? '开启' : '关闭' }}</div>
              </li>
              <li class="item" v-if="orgDetail.positioning_status">
                <div>区域中心：</div>
                <div class="value">{{ orgDetail.address }}</div>
              </li>
            </ul>
          </div>
...
<script>
import { getStoreGroupDetailApi, getStoreGroupStoresApi } from '@/api/store';
...
export default {
  ...
  data() {
    return {
      tagContent: ORG_TYPE_META[ORG_TYPE.GROUP].label,
      ORG_TYPE_META,
      orgDetail: null,
      loading: false,
  ...
  watch: {
    visible(visible) {
      if (visible) {
        this.orgId && this.handleGetZoneDetail(this.orgId);
  ...
  methods: {
    getOrgTypeStyle() {
      return getOrgTypeStyle(ORG_TYPE.GROUP);
    },
    ...
    async handleGetMerList() {
      ...
      try {
        const res = await getStoreGroupStoresApi(this.orgId, this.searchForm);
    ...
    async handleGetZoneDetail() {
      ...
      this.loading = true;
      try {
        const res = await getStoreGroupDetailApi(this.orgId);

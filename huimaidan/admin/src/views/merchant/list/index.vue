  ...
  <div class="divBox">
    <div class="selCard">
      ...
      >
        ...
        <el-form-item label="线下支付：" prop="offline_switch">
          <el-select
            v-model="tableFrom.offline_switch"
            clearable
            placeholder="请选择"
            class="selWidth"
            @change="getList(1),getHeadNum()"
        ...
        <el-form-item>
          <el-button type="primary" size="small" @click="getList(1),getHeadNum()"
            >搜索</el-button>
    ...
    <el-card class="mt14">
      <div class="mb20">
        ...
        <el-button size="small" type="primary" class="mt5" @click="onAdd" :loading="addFormLoading"
          >添加店铺</el-button
        >
      </div>
      <el-table
      ...
      >
        ...
        <el-table-column prop="merchantBusiness.name" label="所属商户" min-width="150" />
        <el-table-column prop="real_name" label="联系人" min-width="150" >
          <template slot-scope="scope">
            <span>{{ scope.row.real_name || '--' }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="merchantType.type_name" label="店铺类型" min-width="120" />
        <el-table-column prop="margin" label="保证金" min-width="120">
          <template slot-scope="scope">
            <span>{{ scope.row.is_margin | marginFilter }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="status" label="推荐" min-width="120">
          <template slot-scope="scope">
            <el-switch
        ...
        <el-table-column prop="status" label="开启/关闭" min-width="120">
          <template slot-scope="scope">
            <el-switch
    ...
    <mer-detail
      ...
      :storeGroupList="storeGroupOptions"
      :storeType="storeType"
      :zoneList="zoneList"
...
import { getStoreCreateFormConfig } from "./add.form";
import { mapGetters } from "vuex";
import { getStoreGroupOptionsApi } from "@/api/store";

export default {
  name: "MerchantList",
  components: { merDetail },
  computed: {
    ...mapGetters('user', ['isMerAdmin', 'zoneId']),
  },
  data() {
    return {
      fromList: fromList,
  ...
  mounted() {
    ...
    this.getStoreType();
    this.getZoneList();
    this.getStoreGroupOptions();
    this.getList("");
  },
  methods: {
    async getStoreGroupOptions() {
      try {
        const res = await getStoreGroupOptionsApi();
        this.storeGroupOptions = res.data;
      } catch (error) {
    ...
    // 修改状态
    onchangeIsShow(row) {
      const title = row.is_best === 1 ? "是否开启推荐店铺" : "是否关闭推荐店铺";
    ...
    // 商户管理员添加店铺
    async handleAddStore(formData) {
      try {
        const res = await createStoreApi({
          ...formData,
          business_id: this.zoneId
        });
    ...
    // 添加
    async onAdd() {
      // 商户管理员使用 form-dialog 弹窗
      if (this.isMerAdmin) {
        this.addFormLoading = true;
        const formConfig = await getStoreCreateFormConfig();
        this.addFormLoading = false;
        this.$refs.formDialog.open({
          title: "添加店铺",
          rule: formConfig,
          action: formData => this.handleAddStore(formData)
        });
      } else {
        // 平台管理员使用 drawer 弹窗
        this.drawer = true;
    ...
    // 设置复制次数
    handleTimes(id) {
      this.$modalForm(changeCopyApi(id)).then(() =>
        this.$refs.merDetail.getInfo(id)
      );
    },
    /**设置虚拟关注数 */
    setCareFicti(id) {
      this.$modalForm(merchantCareFicti(id)).then(() =>
        this.$refs.merDetail.getInfo(id)
      );
    },
    // 修改密码表单

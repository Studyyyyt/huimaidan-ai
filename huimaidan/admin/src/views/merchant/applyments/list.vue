      ...
      <el-table v-loading="listLoading" :data="tableData.data" size="small">
        ...
        <el-table-column label="订单金额" min-width="100">
          <template slot-scope="scope">
            ...
            <div
              v-if="scope.row.profitsharing_refund > 0"
              style="color:#82e493;"
            >
              退款金额： {{ scope.row.profitsharing_refund }}
            </div>
            <div>分账商户所得金额： {{ scope.row.profitsharing_mer_price }}</div>
          </template>
        </el-table-column>
        <el-table-column label=" 账单类型" min-width="100">
          <template slot-scope="scope">
            <span>{{
              scope.row.type == "order" ? "订单支付" : "尾款支付"
            }}</span>
          </template>
        </el-table-column>
        <el-table-column label="状态" min-width="120">
          <template slot-scope="scope">
            ...
            <div v-else-if="scope.row.status == 1">
              已分账<br />分账时间： {{ scope.row.profitsharing_time }}
            </div>
            <div v-else-if="scope.row.status == 2">分帐中</div>
            <div v-else-if="scope.row.status == -1">已退款</div>
            <div v-else-if="scope.row.status == -2">
              分账失败<br />
              <span style="color: red; font-size: 12px;">
                失败原因： {{ scope.row.error_msg }}</span
              >
            </div>
          </template>
        </el-table-column>

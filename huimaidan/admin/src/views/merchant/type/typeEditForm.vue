    ...
    <el-tabs v-loading="loading" type="border-card" v-model="name">
      ...
      <div class="section" style="padding-top: 0">
        <el-form-item label-width="0">
          <div style="color:#F56464;font-size: 13px;">如果提示权限不足，请先去掉任意一个权限提交，然后重新补充权限再次提交即可。</div>
        </el-form-item>
        <div class="title" style="margin-bottom: 15px;">权限管理</div>
        <el-form-item label="权限：" label-width="80px">
          <div class="handle-btn" @click="handleCheckedTreeExpand">展开/收起</div>
        </el-form-item>
        <el-form-item prop="auth" label-width="80px">
          <el-tree
            ref="tree"
            :data="permissions"
            show-checkbox
...
export default {
  ...
  methods: {
    ...
    handleCheckedTreeExpand() {
      ...
      let treeList = this.permissions;
      for (let i = 0; i < treeList.length; i++) {
        this.$refs.tree.store.nodesMap[treeList[i].value].expanded = this.menuExpand;
      }
    },
    // 树权限（全选/全不选）
    handleCheckedTreeNodeAll(value, type) {
      if (type == 'menu') {
        this.$refs.tree.setCheckedNodes(value ? this.permissions : []);
      }
    },
    /*提交信息*/
    onSubmit(id){

        ...
        <el-table-column label="操作" min-width="150" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button
              type="text"
              size="small"
              @click="handleDelete(scope.row.menu_id, scope.$index)"
              >删除</el-button
            >
          </template>
        </el-table-column>
...
import {
  merchantMenuListApi,
  merchantMenuCreateApi,
  merchantMenuUpdateApi,
  merchantMenuDeleteApi
} from "@/api/merchant";
export default {
  name: "MerchantSystem",
  data() {
  ...
  methods: {
    ...
    getList() {
      ...
      merchantMenuListApi(this.tableFrom)
        .then(res => {
    ...
    onAdd(id) {
      ...
      this.$modalForm(merchantMenuCreateApi(), config).then(() =>
        this.getList()
      );
    },
    // 编辑

<template>
  <div class="divBox">
    <div class="selCard">
      <el-form inline size="small" :model="tableFrom" ref="searchForm" label-width="85px">
        <el-form-item label="商品分类：" prop="pid">
          <el-cascader v-model="tableFrom.pid" class="selWidth" :options="merCateList" :props="{ checkStrictly: true, emitPath:false }" clearable @change="getList(1),getLstFilterApi()" />
        </el-form-item>
        <el-form-item label="店铺名称：" prop="mer_id">
          <el-select v-model="tableFrom.mer_id" clearable filterable remote :remote-method="getMerSelect" placeholder="请选择" class="selWidth my-select" @visible-change="getMerSelect()" @change="getMerSelect(),getList(1),getLstFilterApi()">
        ...
        <el-form-item label="店铺类别：" prop="is_trader">
          <el-select v-model="tableFrom.is_trader" clearable placeholder="请选择" class="selWidth" @change="getList(1),getLstFilterApi()">
            <el-option label="自营" value="1" />
            <el-option label="非自营" value="0" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="tableFrom.type == 2"  label="商品状态：" prop="us_status">
          <el-select v-model="tableFrom.us_status" placeholder="请选择" class="filter-item selWidth" clearable @change="getList(1),getLstFilterApi()">
            <el-option v-for="item in productStatusLists" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item v-else label="商品状态：" prop="us_status">
          <el-select v-model="tableFrom.us_status" placeholder="请选择" class="filter-item selWidth" clearable @change="getList(1),getLstFilterApi()">
        ...
        <el-form-item label="推荐级别：" prop="star">
          <el-select v-model="tableFrom.star" placeholder="请选择" class="filter-item selWidth" clearable @change="getList(1),getLstFilterApi()">
        ...
        <el-form-item label="商品标签：" prop="sys_labels">
          <el-select v-model="tableFrom.sys_labels" placeholder="请选择" class="filter-item selWidth" clearable filterable @change="getList(1),getLstFilterApi()">
        ...
        <el-select v-model="tableFrom.svip_price_type" placeholder="请选择" class="selWidth" clearable @change="getList(1),getLstFilterApi()">
      ...
      <el-form-item label="商品推荐：" prop="hot_type">
        <el-select v-model="tableFrom.hot_type" placeholder="请选择" class="filter-item selWidth" clearable filterable @change="getList(1),getLstFilterApi()">
      ...
      <el-form-item label="大图推荐：" prop="cate_hot">
        <el-select v-model="tableFrom.cate_hot" placeholder="请选择" class="filter-item selWidth" clearable filterable @change="getList(1),getLstFilterApi()">
          <el-option label="开启" value="1" />
          <el-option label="关闭" value="0" />
        </el-select>
      </el-form-item>
      <el-form-item label="商品类型：" prop="is_ficti">
        <el-select v-model="tableFrom.is_ficti" placeholder="请选择" class="filter-item selWidth" clearable @change="getList(1),getLstFilterApi()">
      ...
      <el-form-item label="商品搜索：" prop="keyword">
        <el-input v-model="tableFrom.keyword" @keyup.enter.native="getList(1),getLstFilterApi()" placeholder="请输入商品名称，关键字，产品编号" class="selWidth" />
      </el-form-item>
      <el-form-item>
    ...
    <el-card class="mt14">
      <el-tabs v-model="tableFrom.type" @tab-click="tabClickFn" >
      ...
      <div class="mt5 mb20">
        ...
        <el-button :disabled="multipleSelection.length==0" size="small" @click="handleBatchCopyProduct">批量复制商品</el-button>
      </div>
      <el-table v-loading="listLoading" :data="tableData.data" size="small" @selection-change="handleSelectionChange">
        <el-table-column v-if="Number(tableFrom.type)<7 && tableFrom.type!=5" key="2" type="selection" width="55" />
        <el-table-column type="expand">
          <template slot-scope="props">
            <el-form label-position="left" inline class="demo-table-expand" label-width="fit-content" >
              ...
              <el-form-item label="活动标签：" v-if="props.row.activity_labels.length" class="form-item-flex">
        ...
        <el-table-column v-if="tableFrom.type != 5" :key="1" prop="status" label="是否显示" min-width="80">
          <template slot-scope="scope">
        ...
        <el-table-column v-if="Number(tableFrom.type) < 7" key="8" label="操作" :min-width="(Number(tableFrom.type) < 7 && tableFrom.type !=5) ? '180' : '100'" fixed="right">
          <template slot-scope="scope">
            ...
            <el-dropdown v-if="Number(tableFrom.type) < 7 && tableFrom.type !=5">
              <span class="el-dropdown-link">
                  更多<i class="el-icon-arrow-down el-icon--right" />
              </span>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-if="Number(tableFrom.type) < 7 && tableFrom.type !=5" @click.native="onEdit(scope.row.product_id)">编辑商品</el-dropdown-item>
                <el-dropdown-item v-if="tableFrom.type != 5" style="font-size: 14px;">
                  <router-link :to="{path: roterPre + '/product/comment?product_id=' + scope.row.product_id}">
                    查看评价
                  </router-link>
                </el-dropdown-item>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" size="small" @click="submitRecommendForm">提交</el-button>
      </span>
    </el-dialog>
    <!--商品详情-->
    <pro-detail
    ...
    <mer-select ref="merSelect" @confirm="handleBatchCopyProductConfirm" />
  </div>
</template>
...
import { productBatchCopyApi } from '@/api/product';
...
export default {
  ...
  computed: {
    activityLabelCascader() {
      const hasChildrenCategory = this.activityLabelCateList.filter(item => item.children && item.children.length);
      return this.formatActivityLabelCascader(hasChildrenCategory);
    }
  },
  mounted() {
    this.getActivityLabelCateList();
    this.getMerSelect();
    this.getList('')
  ...
  methods: {
    async handleBatchCopyProductConfirm(storeId) {
      if (!this.OffId.length || !storeId) return;
      const payload = {
        store_id: storeId,
        product_ids: this.OffId
      };
      
      try {
        const res = await productBatchCopyApi(payload);
    ...
    // 批量复制商品
    handleBatchCopyProduct() {
      this.$refs.merSelect.open();
    },
    ...
    async getActivityLabelCateList() {
      try {
        const res = await activityLabelTreeApi();
        this.activityLabelCateList = res.data.map(item => {
          return {
            ...item,
            id: `cate_${item.id}`
          };
    ...
    submitRecommendForm() {
      ...
      this.recommendForm.cate_hot = this.cate_hot
    ...
    // 店铺列表；
    getMerSelect(query) {
      merSelectListApi({keyword: query}).then(res => {

...
export default {
  ...
  methods: {
    getInfo() {
      ...
      getBrokerageApi('promoter_explain').then(res => {
        const info = res.data
        this.formValidate = {
          agree: info.promoter_explain,
        }
    ...
    handleSubmit(name) {
      if(this.formValidate.agree === '' || !this.formValidate.agree){
        this.$message.warning("请输入说明信息！");
        return
      }else{
        updateBrokerageApi('promoter_explain', this.formValidate).then(async res => {
          this.fullscreenLoading = false

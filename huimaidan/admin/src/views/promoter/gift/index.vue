    ...
    <div class="selCard">
      <el-form :model="tableFrom" ref="searchForm" inline size="small" label-width="85px">
        ...
        <el-form-item label="店铺名称：" prop="mer_id">
          <el-select v-model="tableFrom.mer_id" clearable filterable placeholder="请选择" class="selWidth" @change="getList(1),getLstFilterApi()">
            <el-option
        ...
        <el-form-item label="商品搜索：" prop="keyword">
          <el-input v-model="tableFrom.keyword" @keyup.enter.native="getList(1),getLstFilterApi()" placeholder="请输入商品名称，关键字，产品编号" class="selWidth" clearable />
        </el-form-item>
        <el-form-item>
...
export default {
  ...
  components: { infoFrom, WangEditor },
  data() {
    return {
      props: {
        emitPath: false
  ...
  methods: {
    ...
    // 列表表头；
    getLstFilterApi() {
      lstFilterApi(this.tableFrom).then(res => {

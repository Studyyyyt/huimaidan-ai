  ...
  <div class="divBox">
    ...
    <el-card class="mt14">
      ...
      <el-table v-loading="loading" :data="tableData.data" size="small">
        ...
        <el-table-column prop="nickname" label="客服名称" min-width="150" />
        <el-table-column label="帐号状态" min-width="120">
          <template slot-scope="scope">
            <div class="switch-width-double">
              <el-switch
        ...
        <el-table-column label="操作" min-width="180" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button
              type="text"
              size="small"
              @click="goList(scope.row.service_id, scope.$index)"
              >聊天记录</el-button
            >
            <el-button
              ...
              type="text"
              size="small"
              @click="onEdit(scope.row.service_id)"
              >编辑</el-button
            >
            <el-button
              type="text"
              size="small"
              @click="onDel(scope.row.service_id, scope.$index)"
              >删除</el-button
            >
          </template>
        </el-table-column>
    ...
    >
        ...
        <el-table-column label="操作" fixed="right" min-width="80">
          <template slot-scope="scope">
            <el-button
              type="text"
              size="small"
              @click="check(scope.row.uid, scope.$index)"
              >查看对话</el-button
            >
          </template>
        </el-table-column>
      ...
      >
        ...
        <el-table-column label="发送消息" min-width="100">
          <template slot-scope="scope">
            <span v-if="scope.row.msn_type === 2" class="tabImage"
              >[{{ scope.row.msn }}]</span
            >
            <img
              v-else-if="scope.row.msn_type === 3"
              class="tabImage"
              :src="scope.row.msn"
            />
...
export default {
  ...
  methods: {
    ...
    onchangeIsShow(row) {
      ...
      serviceStatusApi(row.service_id, row.is_open)
        .then(({ message }) => {
          this.$message.success(message);
          this.getList();
        })

  ...
  <div class="divBox">
    <el-card>
      ...
      <el-table
        ...
        row-key="topic_id"
      >
        <el-table-column label="话题名称" min-width="100">
        ...
        <el-table-column label="上级分类" min-width="100">
          <template slot-scope="scope">
            <span>{{
              scope.row.category && scope.row.category.cate_name
            }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="sort" label="排序" min-width="100" />
        <el-table-column prop="count_use" label="文章数" min-width="100" />
        ...
        <el-table-column label="操作" min-width="100" fixed="right">
          <template slot-scope="scope">
            <el-button
              type="text"
              size="small"
              @click="onEdit(scope.row.topic_id)"
              >编辑</el-button
            >
            <el-button
              type="text"
              size="small"
              @click="handleDelete(scope.row.topic_id, scope.$index)"
              >删除</el-button
            >
          </template>
        </el-table-column>
...
// +----------------------------------------------------------------------
import {
  communityTopicListApi,
  communityTopicCreateApi,
  communityTopicUpdateApi,
  communityTopicDeleteApi,
  communityTopicStatusApi,
  communityTopicHotApi
} from "@/api/community";
export default {
  name: "communityTopic",
  data() {
  ...
  methods: {
    ...
    onchangeIsShow(row) {
      communityTopicStatusApi(row.topic_id, row.status)
        .then(({ message }) => {
          this.$message.success(message);
        })
        .catch(({ message }) => {
    ...
    onchangeIsHot(row) {
      communityTopicHotApi(row.topic_id, row.is_hot)
        .then(({ message }) => {
          this.$message.success(message);
        })
        .catch(({ message }) => {

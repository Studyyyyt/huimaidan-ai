<template>
  <div class="divBox">
    <el-card class="box-card">
      ...
      >
        <el-table-column
          prop="activity.activity_name"
          label="活动名称"
          min-width="100"
        />
        <el-table-column label="用户名称/ID" min-width="100">
          <template slot-scope="{ row }">
            <div>{{ row.nickname }}/{{ row.uid }}</div>
          </template>
        </el-table-column>
        ...
        >
          <template slot-scope="{ row }">
            <div v-if="item.type == 'uploadPicture'">
              <el-image
                v-for="(img, i) in row.value[item.key]"
                :key="i"
            ...
            <div v-else-if="item.type == 'dateranges'">
              <div v-if="row.value[item.key]">
                {{ row.value[item.key][0] }}至<br />{{ row.value[item.key][1] }}
              </div>
            </div>
            <div v-else>
              {{ row.value[item.key] }}
...
export default {
  ...
  methods: {
    ...
    async onExport() {
      ...
      let excelData = JSON.parse(JSON.stringify(params)),
        data = [];
      excelData.page = 1;
      let pageCount = 1;
      let lebData = {};

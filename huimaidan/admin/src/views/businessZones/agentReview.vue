  ...
  <div class="divBox">
    ...
    <el-card class="mt14">
      ...
      <el-table :data="agentList" size="small" v-loading="formLoadOptions.loading">
        ...
        <el-table-column label="代理操作" prop="action">
          <template #default="{ row }">
    ...
    <!-- 代理详情面板 -->
    <agent-review-detail :visible.sync="agentReviewDetailVisible"
      :agentId="currentAgent && currentAgent.circle_agent_id" @close="handleCloseAgentReviewDetail"
...
import { AGENT_STATUS_LIST, AGENT_STATUS_MAP, USER_SEARCH_OPTIONS } from './domain/agent.props.js';
import { getInitSearchForm } from './domain/agent.utils.js';
import { getInitFormLoadOptions } from './utils.js';
import AgentReviewDetail from './components/agentReviewDetail.vue';
...
export default {
  ...
  data() {
    return {
      ...
      agentReviewDetailVisible: false, // 代理详情面板是否可见
  ...
  methods: {
    ...
    // 打开代理详情面板
    handleAgentReviewDetail(agent) {
      this.currentAgent = agent;
      this.agentReviewDetailVisible = true;
    },
    // 关闭代理详情面板
    handleCloseAgentReviewDetail() {
      this.agentReviewDetailVisible = false;
      this.currentAgent = null;
    },
    // 通过审核
    async handleReview(agent) {
      try {
        await this.$confirm('您确定要通过此代理审核吗？', '提示', {
      ...
      try {
        const res = await updateAgentStatusApi(agent.circle_agent_id, {
    ...
    async handleConfirmReject() {
      ...
      try {
        const res = await updateAgentStatusApi(this.currentAgent.circle_agent_id, {

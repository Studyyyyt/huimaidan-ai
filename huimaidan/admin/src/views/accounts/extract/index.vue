      ...
      >
        ...
        <el-table-column label="二维码" min-width="80">
          <template slot-scope="scope">
            <div class="demo-image__preview">
              <el-image
                v-if="scope.row.extract_pic"
                :src="scope.row.extract_pic"
                :preview-src-list="[scope.row.extract_pic]"
              />
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="账号" min-width="100">
          <template slot-scope="scope">
            ...
            <span
              v-else-if="
                scope.row.extract_type == 2 || scope.row.extract_type == 3
              "
              >{{ scope.row.wechat }}</span
            >
            <span v-else>-</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" min-width="80" fixed="right">
          <template slot-scope="scope">
            <el-button
              ...
              type="text"
              size="small"
              @click="onAudit(scope.row.extract_id)"
              >审核</el-button
            >
            <el-button
              v-else
      ...
      <div v-loading="loading" style="margin-top: 5px;">
        <div class="box-container">
          <div class="list-count">
            ...
            <div class="acea-row">
              ...
              <div class="list">
                ...
                <span v-if="extractDetail.extract_type == 0">{{
                  extractDetail.bank_code
                }}</span>
                <span v-else-if="extractDetail.extract_type == 1">{{
                  extractDetail.alipay_code
                }}</span>
                <span
                  v-else-if="
                    extractDetail.extract_type == 2 ||
                      extractDetail.extract_type == 3
                  "
                  >{{ extractDetail.wechat }}</span
                >
                <span v-else>-</span>
              </div>
          ...
          <div class="list-count">
            ...
            <div class="acea-row">
              <div class="list">
                <label class="name">提现方式：</label
                >{{ extractDetail.extract_type | extractTypeFilter }}
              </div>
              <div class="list">
...
export default {
  ...
  methods: {
    ...
    /**审核 */
    onAudit(id) {
      this.$modalForm(extractManageAudit(id)).then(() => this.getList());
    },
    onDetails(id) {

      ...
      >
        <el-table-column label="订单号" min-width="200">
          <template slot-scope="scope">
            <span v-if="scope.row.financial_type != 'sys_accoubts'">{{
              scope.row.order_sn
            }}</span>
        ...
        <el-table-column label="第三方交易单号" min-width="200">
          <template slot-scope="scope">
            <span>{{
              (scope.row.orderInfo && scope.row.orderInfo.transaction_id) || "-"
            }}</span>
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column label="操作" min-width="150" fixed="right">
          <template slot-scope="scope">
            <router-link
              v-if="scope.row.financial_type == 'sys_accoubts'"
              :to="{
                path:
                  `${roterPre}` +
            ...
            >
            <router-link
              v-else-if="scope.row.financial_type == 'svip'"
              :to="{ path: `${roterPre}` + '/user/member/record' }"
            >
              <el-button type="text" size="small" class="mr10">详情</el-button>
            </router-link>
            <router-link
...
import orderDetail from "../../order/list/orderDetails.vue";
...
export default {
  ...
  methods: {
    ...
    //获取统计数据
    getStatisticalData() {
      getStatisticsApi({ date: this.tableFrom.date })
        .then(res => {

  ...
  computed: {
    ...
    btnList() {
      ...
      const config = [
        {
          text: "转账",
          condition: () => this.review.audit_status === AGENT_STATUS.APPROVED && this.review.status === CREDIT_STATUS.PENDING,
          action: () => this.$emit("transfer", this.review),
          type: "primary"
        },
        {
          text: "同意",
          condition: () => this.review.audit_status === AGENT_STATUS.PENDING,
          action: () => this.$emit("pass", this.review),
          type: "primary"
        },
        {
          text: "拒绝",
          condition: () => this.review.audit_status === AGENT_STATUS.PENDING,
          action: () => this.$emit("reject", this.review),
        },
        {
          text: "备注",
          condition: () => true,

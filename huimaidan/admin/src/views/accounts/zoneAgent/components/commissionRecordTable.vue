<template>
  <el-table :data="list" size="small" v-loading="loading" class="mt-14">
    <el-table-column v-for="item of TABLE_FIELD_OPTIONS" :key="item.field" :prop="item.field" :label="item.label"
      ...
      <template v-else-if="item.labelList" #default="{ row }">
        {{ item.formatter(row) }}
        <span v-for="labelItem of item.labelList(row)" :key="labelItem.label" :style="{ color: labelItem.color }">{{
          labelItem.label }}</span>
      </template>

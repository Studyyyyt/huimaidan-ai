<template>
  <div class="divBox">
    ...
    <el-table
      ...
      tooltip-effect="dark"
    >
      <el-table-column v-if="!newGift" key="2" width="55">
      ...
      <el-table-column v-if="newGift" key="3" width="55">
        ...
        <template slot-scope="scope">
          <el-checkbox :value="checkIds.indexOf(scope.row.coupon_id) > -1" @change="(v) => changeOne(v, scope.row)" />
        </template>
      </el-table-column>
      <el-table-column
        prop="coupon_id"
      ...
      <el-table-column v-if="!newGift" key="1" label="操作" min-width="120" fixed="right">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    changeOne(v, row) {
      if (v) {
        const index = this.checkIds.indexOf(row.coupon_id);
        if (index === -1) {
          this.checkIds.push(row.coupon_id);
    ...
    syncCheckedId(o) {
      const ids = this.tableData.data.map((v) => v.coupon_id);
      if (o) {
        this.tableData.data.forEach((item) => {
          const index = this.checkIds.indexOf(item.coupon_id);
          if (index === -1) {
            this.checkIds.push(item.coupon_id);
    ...
    ok(){
      this.tableFrom.coupon_name = "";
      this.$emit('sendSuccess', this.checkBox, this.checkIds);
    },
    close(){
      this.$emit('close');
    },
    ...
    getList(num) {
      ...
      platformLstApi(this.tableFrom).then(res => {
        ...
        if(this.newGift)this.setCheckedProduct(this.checkedData);

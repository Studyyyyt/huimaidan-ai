    ...
    <el-card class="mt14">
      <div class="mb20">
        ...
        <el-button
          ...
          >导出搜索记录</el-button
        >
        <el-button size="small" class="mt5" @click="clear">一键清空</el-button>
      </div>
      <el-table
      ...
      >
          ...
          <template slot-scope="scope">
            <div class="demo-image__preview">
              <el-image
                ...
                :src="scope.row.user ? scope.row.user.avatar : moren"
                :preview-src-list="[
                  (scope.row.user && scope.row.user.avatar) || moren
                ]"
              />
            </div>
          </template>
        ...
        <el-table-column label="昵称" min-width="120">
          <template slot-scope="{ row }">
            <div class="acea-row">
              <div>
                {{ row.user && row.user.nickname ? row.user.nickname : "未知" }}
              </div>
            </div>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList(num) {
      ...
      userSearchLstApi(this.userFrom)
        .then(res => {

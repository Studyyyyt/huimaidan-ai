<template>
  <div class="divBox">
    ...
    <el-card class="mt14">
      ...
      >
        <el-table-column label="ID" width="80" prop="template_id" />
        <el-table-column prop="tempid" label="模板ID" min-width="220" />
        ...
        <el-table-column label="操作" min-width="100" fixed="right">
          <template slot-scope="scope">
            <el-button
              type="text"
              size="small"
              @click="onEdit(scope.row.template_id)"
              >编辑</el-button
            >
            <el-button
              type="text"
...
export default {
  ...
  methods: {
    ...
    handleDelete(id, idx) {
      this.$modalSure().then(() => {
        this.$route.path.indexOf("wechat") !== -1
    ...
    onchangeIsShow(row) {
      this.$route.path.indexOf("wechat") !== -1
        ? templateStatusApi(row.template_id, { status: row.status })
            .then(() => {
        ...
        : routineStatusApi(row.template_id, { status: row.status })
            .then(() => {
              this.$message.success("修改成功");
              this.getList();
            })

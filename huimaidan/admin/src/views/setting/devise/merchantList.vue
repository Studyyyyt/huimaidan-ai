        ...
        <el-card class="mt14">
          <div class="acea-row">
            <div style="width: 310px;height:550px;margin-right: 30px;position: relative">
              <iframe class="iframe-box" v-if="previewUrl" :src="previewUrl" frameborder="0" ref="iframe"></iframe>
              <!-- <div class="mask"></div> -->
            </div>
            <div class="table">
                ...
                <div type="flex">
                  <div>
                    <div class="acea-row row-between-wrapper">
                      ...
                      <div style="color:#F56464;font-size: 13px;">&nbsp;&nbsp;注：初次进入该页面，可点击[添加]创建店铺首页模板，点击[适用范围] 可将该模板指定给相应店铺使用。</div>
                    </div>
                  </div>
                </div>
              </div>
      ...
      >
        <el-form-item label-width="0">
          <el-radio-group v-model="formValidate.scope_type" @change="getChange()">
            ...
            <el-radio :label="1">指定店铺分类使用</el-radio>
            <el-radio :label="2">指定店铺类型使用</el-radio>
            <el-radio :label="3">指定店铺类别使用</el-radio>
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item v-if="formValidate.scope_type == 1" label="店铺分类："  prop="scope_value">
           <el-select
        ...
        <el-form-item v-if="formValidate.scope_type == 3" label="店铺类别：" prop="scope_value">
          <el-select v-model="formValidate.scope_value" clearable filterable multiple placeholder="请选择" class="selWidth">
...
export default {
  ...
  computed: {
    ...
    previewUrl() {
      ...
      if (!this.previewDiyId || !this.merSelect.length) return null;
      return `${this.BaseURL}/pages/admin/storeDiy/index?id=${this.merSelect[0].mer_id}&diy_id=${this.previewDiyId}&inner_frame=1&time=${time}`
    }
  },
  components: {},
  data() {
    return {
      ...
      list: [],
      modal: false,
      BaseURL: isDev ? 'http://localhost:8080' : SettingMer.httpUrl,
      rangeLoading: false,
      diyFrom: {
        page: 1,
        limit: 10,
  ...
  methods: {
    ...
    getList() {
      ...
      merchantDiyList(this.diyFrom).then((res) => {
        ...
        if (data.list.length) {
          this.previewDiyId = data.list[0]['id'];
        }
      });
    },
    pageChange(status) {

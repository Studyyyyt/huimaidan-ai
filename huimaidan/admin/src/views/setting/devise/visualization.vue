    ...
    <el-card :bordered="false" dis-hover class="ivu-mt">
      <el-row class="ivu-mt box-wrapper">
        <el-col
          ...
          :lg="4"
          class="left-wrapper"
          v-if="!$route.params.id && groupAll.length"
        >
          <!-- 左侧菜单 -->
          <el-tabs
        ...
        <!-- 自定义配置页面展示 -->
        <el-col
          v-if="name == 'integral_shop_banner'"
          class="iframe"
          :bordered="false"
        >
          <div class="iframe-box">
            <img
              src="../../../assets/images/integral.png"
              style="width: 100%;"
            />
          ...
          <div
            v-if="
              name == 'new_home_banner' ||
                name == 'hot_home_banner' ||
                name == 'best_home_banner' ||
                name == 'good_home_banner' ||
                name == 'points_mall_banner'
            "
            class="moddile_box"
          >
            <div class="nofonts" v-if="tabList.list == ''">
        ...
        <el-col v-if="name == 'sign_day_config'" class="iframe">
          <div class="iframe-box">
            <img
              v-if="bgimg == 0"
              src="../../../assets/images/purplesign.png"
            />
        ...
        <el-col v-if="guide == 2">
          <div v-if="guide == 2" class="iframe" :bordered="false">
            ...
            <swiper :options="swiperOption" class="swiperimg on">
              <swiper-slide
                class="swiperimg on"
                v-for="(item, indexa) in tabList.list"
                :key="indexa + 'a'"
              >
        ...
        <el-col
          v-if="name == 'sign_day_config'"
          :xs="24"
          :sm="24"
          :md="14"
          :lg="11"
          class="ml20 right-wrapper"
        >
          <div class="table_box">
            <el-row type="flex">
              <el-col v-bind="grid">
            ...
            <div class="table">
              ...
              >
                ...
                <el-table-column key="5" prop="sort" label="排序" width="80">
                </el-table-column>
          ...
          <div class="table_box">
            ...
            <div class="table">
              ...
              >
                ...
                <el-table-column key="18" prop="title" label="标题" width="120">
                </el-table-column>
                <el-table-column
                  key="7"
                  prop="sign_day"
                ...
                <el-table-column key="9" prop="sort" label="排序" width="80">
                </el-table-column>
          ...
          <div class="content">
            <div class="ml20">
              <div class="hot_imgs">
                    ...
                    <el-form :model="formItem" label-width="80px">
                      ...
                      <el-form-item label="间隔时间:">
                        ...
                        <div style="color:#999999;">
                          备注：弹广告的频次，即N小时内进入不再显示广告页，0为每次进入都显示
                        </div>
                      </el-form-item>
                    </el-form>
                  </div>
                ...
                <template>
                  <div class="add-btn">
                    <el-button class="btn-add" size="small" @click="addBox"
                      >添加图片</el-button
                    >
                  </div>
                </template>
        ...
        <el-col v-else>
          <!-- 右侧轮播图以及其他设置 -->
          <div
            v-if="name != 'sign_day_config' && name != 'points_mall_scope'"
            class="content"
          >
            <div class="ml20">
              <div class="hot_imgs">
                <div class="title">
                  {{
                    name != "points_mall_district"
                      ? "轮播图设置"
                      : "积分金刚区设置"
                  }}
                </div>
                <div v-if="name != 'points_mall_district'" class="title-text">
                  建议尺寸：690 * 240px，拖拽图片可调整图片顺序哦，最多添加五张
                </div>
                <div v-else class="title-text">
                  建议尺寸：90 *
                  90px，拖拽图片可调整图片顺序哦，图片名称建议四个字以内
                </div>
                <div class="list-box">
                  <draggable
                    class="dragArea list-group"
...
export default {
  ...
  mounted() {
    ...
    this.url =
      this.BaseURL +
      "/pages/columnGoods/HotNewGoods/index?inner_frame=1&type=new&time=" +
      time;
  },
  methods: {
      ...
      if (this.name == "open_screen_advertising") {
        ...
        switch (this.name) {
          case "best_home_banner":
            this.url = `${
              this.BaseURL
            }/pages/columnGoods/HotNewGoods/index?inner_frame=1&type=best&time=${time}`; //精品推荐
            break;
          ...
          case "points_mall_scope":
            this.url = `${
              this.BaseURL
            }/pages/points_mall/index?inner_frame=1&time=${time}`; //积分商城
            break;
          case "hot_home_banner":
            this.url = `${
              this.BaseURL
            }/pages/columnGoods/HotNewGoods/index?inner_frame=1&type=hot&time=${time}`; //热门榜单
            break;
          case "new_home_banner":
            this.url = `${
              this.BaseURL
            }/pages/columnGoods/HotNewGoods/index?inner_frame=1&type=new&time=${time}`; //首发新品
            break;
          case "good_home_banner":
            this.url = `${
              this.BaseURL
            }/pages/columnGoods/HotNewGoods/index?inner_frame=1&type=good&time=${time}`; //推荐单品
            break;
          case "points_mall_district":
            this.url = `${
              this.BaseURL
            }/pages/points_mall/index?inner_frame=1&time=${time}`; //积分金刚区
            break;
          case "points_mall_banner":
            this.url = `${
              this.BaseURL
            }/pages/points_mall/index?inner_frame=1&time=${time}`; //积分商城轮播图
            break;
        }
      }
    },
    addBox() {
      if (this.tabList.list.length == 0) {
        ...
        if (
          this.tabList.list.length == 5 &&
          this.name != "points_mall_district"
        ) {
    ...
    // 编辑
    edit(row) {
      this.$modalForm(groupDataEditApi(this.pageId, row.group_data_id)).then(
        () => {
          this.info(this.name);
    ...
    del(row) {
      this.$modalSure("删除数据吗").then(() => {
        groupDataDeleteApi(row.group_data_id)
          .then(({ message }) => {

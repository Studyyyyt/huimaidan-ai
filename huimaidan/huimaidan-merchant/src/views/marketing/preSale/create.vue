      ...
      >
        ...
        <div v-show="currentTab === 1">
          ...
          <el-row :gutter="24">
            <el-col :span="24">
              <el-form-item label="预售设置：">
                <el-radio-group
                  v-model="formValidate.presell_type"
                  @change="wayChange"
                >
          ...
          <el-row :gutter="24">
            ...
            <el-col :span="24">
              <el-form-item label="发货时间：" prop="delivery_type">
                <div class="acea-row">
                  <el-select
                    v-if="formValidate.presell_type === 1"
                    v-model="formValidate.delivery_type"
                    size="small"
                    placeholder="请选择"
                    class="selWidthd1 mr20"
                  >
                    <el-option
                  ...
                  <el-input-number
                    v-model="formValidate.delivery_day"
                    :min="1"
                    placeholder="请输入天数"
                    size="small"
                    class="mr10"
            ...
            <el-col :span="24">
              <el-form-item label="平台保障服务：">
                <div class="acea-row">
                  ...
                  <el-button class="mr15" size="small" @click="addServiceTem"
                    >添加服务说明模板</el-button
                  >
                </div>
              </el-form-item>
              ...
              >
                ...
                >
                  ...
                  <el-table-column align="center" label="预售价" min-width="80">
                    <template slot-scope="scope">
                      <el-input
                        v-model.number="scope.row['presell_price']"
                        type="number"
                        size="small"
                        class="priceBox"
                        :min="0"
                  ...
                  <el-table-column
                    v-if="formValidate.presell_type === 2"
                    align="center"
                    label="预售定金"
                    min-width="80"
                  >
                    <template slot-scope="scope">
                      <el-input
                        @input="handleInput(scope.row['down_price'])"
                        v-model="scope.row['down_price']"
                        size="small"
                        type="number"
                        class="priceBox"
                        :min="0"
                        :max="scope.row['price'] * 0.2"
                        @blur="restrictedRange(scope.row)"
                      />
                    </template>
                  </el-table-column>
                  <el-table-column
                  ...
                  >
                    <template slot-scope="scope">
                      <span>{{
                        scope.row["presell_price"] && scope.row["down_price"]
                          ? (
                  ...
                  <el-table-column align="center" label="限量" min-width="80">
                    <template slot-scope="scope">
                      <el-input
                        ...
                        :min="0"
                        @change="limitInventory(scope.row)"
                      />
                    </template>
                  </el-table-column>
                  <el-table-column
          ...
          <el-row>
            <el-col
              v-if="
                formValidate.spec_type === 1 && formValidate.attr.length > 0
              "
              :span="24"
              class="noForm"
            >
              <el-form-item label="批量设置：">
                <el-table
                  :data="oneFormBatch"
                  border
                  class="tabNumWidth disabledTable"
                  size="mini"
                >
                  <el-table-column
                    align="center"
                  ...
                  >
                    <template slot-scope="scope">
                      <div>
                        ...
                        <el-input
                          v-else-if="
                            formThead[iii].title === '预售定金' &&
                              formValidate.presell_type === 2
                          "
                        ...
                        <el-input
                          ...
                          v-model="scope.row[iii]"
                          :disabled="
                            formThead[iii].title !== '预售价' &&
                              formThead[iii].title !== '限量'
                          "
                          type="number"
                          size="small"
                          min="0"
              ...
              >
                ...
                <template v-if="manyTabDate">
                  ...
                  >
                    <template slot-scope="scope">
                      <span
                        class="priceBox"
                        :class="scope.row.select ? 'selectOn' : ''"
                ...
                >
                  <template slot-scope="scope">
                    <el-input
                      @input="
                        handleInput(scope.row['down_price'], scope.$index)
                      "
                      :precision="2"
                      size="small"
                      v-model="scope.row['down_price']"
...
export default {
  ...
  data() {
    return {
      ...
      ruleValidate: {
        ...
        pay_count: [
          { required: true, message: "请输入限购量", trigger: "blur" }
        ],
        store_info: [
  ...
  methods: {
      ...
      if (value != "") {
        if (value.indexOf(".") > -1) {
          if (this.formValidate.spec_type == 1) {
            if (index == -1) {
              this.oneFormBatch[0]["down_price"] = value.slice(
                0,
                value.indexOf(".") + 3
              );
            } else {
              this.ManyAttrValue[index]["down_price"] = value.slice(
                0,
                value.indexOf(".") + 3
              );
            }
          } else {
            this.OneattrValue[0]["down_price"] = value.slice(
              0,
              value.indexOf(".") + 3
            );
          }
    ...
    onchangeTime2(e) {
      this.timeVal2 = e;
      this.formValidate.final_start_time = e
        ? this.moment(e[0]).format("YYYY-MM-DD HH:mm:ss")
        : "";
      this.formValidate.final_end_time = e
        ? this.moment(e[1]).format("YYYY-MM-DD HH:mm:ss")
        : "";
    },
    // 设置标签页标题
    ...
    batchAdd() {
      for (const val of this.ManyAttrValue) {
        if (val.select) {
          if (this.oneFormBatch[0].attr != "")
            this.$set(val, "attr", this.oneFormBatch[0].attr);
          if (this.oneFormBatch[0].image != "")
            this.$set(val, "image", this.oneFormBatch[0].image);
          if (
            this.oneFormBatch[0].price != null &&
            this.oneFormBatch[0].price != ""
          )
            this.$set(val, "price", this.oneFormBatch[0].price);
          if (
            this.oneFormBatch[0].cost != null &&
            this.oneFormBatch[0].cost != ""
          )
            this.$set(val, "cost", this.oneFormBatch[0].cost);
          if (
            this.oneFormBatch[0].down_price != null &&
            this.oneFormBatch[0].down_price != ""
          )
            this.$set(val, "down_price", this.oneFormBatch[0].down_price);
          if (
            this.oneFormBatch[0].presell_price != null &&
            this.oneFormBatch[0].presell_price != ""
          )
            this.$set(val, "presell_price", this.oneFormBatch[0].presell_price);
          if (
            this.oneFormBatch[0].stock != null &&
      ...
      this.$route.params.id
        ? preSaleProDetailApi(id)
            .then(async res => {
              ...
              this.formValidate = {
                ...
                guarantee_template_id: mateName(
                  this.guaranteeList,
                  "guarantee_template_id",
                  info.product.guarantee_template_id
                ),
              ...
              if (this.formValidate.spec_type === 0) {
                ...
                this.OneattrValue.forEach((val, i) => {
                  ...
                  this.$set(
                    this.OneattrValue[i],
                    "down_price",
                    val.presellSku ? val.presellSku.down_price : 0
                  );
                  this.$set(
                    this.OneattrValue[i],
                    "presell_price",
                    val.presellSku ? val.presellSku.presell_price : val.price
                  );
                  this.$set(
                    this.OneattrValue[i],
                    "stock",
                    val.presellSku ? val.presellSku.stock : val.old_stock
                  );
                });
                ...
                this.ManyAttrValue.forEach((val, i) => {
                  ...
                  if (val.presellSku) {
                    this.multipleSpecifications = JSON.parse(
                      JSON.stringify(info.product.attrValue)
                    );
                    arr.push(val);
                  }
              ...
              if (info.presell_type == 2) {
                this.timeVal2 = [
                  new Date(res.data.final_start_time),
                  new Date(res.data.final_end_time)
                ];
              }
              ...
              this.$store.dispatch("settings/setEdit", true);
              this.getAttr();
              this.productCon(info.product.type);
              this.getEditorContent(info.product.content.content)
            })
            .catch(res => {
              this.fullscreenLoading = false;
              this.$message.error(res.message);
        ...
        : productDetailApi(id)
            .then(async res => {
              ...
              this.getAttr();
              this.productCon(info.type);
              this.getEditorContent(info.content)
            })
            .catch(res => {
              this.fullscreenLoading = false;
              this.$message.error(res.message);
    ...
    handleSubmit(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          if (this.$route.params.id) {
            if (this.formValidate.presell_type === 1) {
              this.formValidate.final_start_time = this.formValidate.final_end_time =
                "";
            }

<template>
  <div class="divBox">
    ...
    <div class="selCard mt14 mb14">
      <el-form :model="tableFrom" ref="searchForm" inline size="small" label-width="85px" @submit.native.prevent>
        ...
        <el-form-item label="搜索：" prop="keyword">
          <el-input
            ...
            :placeholder="type=='product' ? '请输入商品名称/ID' : '请输入用户姓名/手机/UID'"
            class="selWidth"
          />
        </el-form-item>
    ...
    <el-card>
      ...
      <el-table
        ...
        size="small"
      >
        <el-table-column key="11" prop="nickname" label="用户姓名" min-width="90">
          <template slot-scope="scope">
            <span v-if="scope.row.uid">[{{scope.row.uid}}]</span>{{scope.row.nickname}}
          </template>
        </el-table-column>
        <el-table-column key="10" prop="sum_total_num" label="购买件数" min-width="80" />
        <el-table-column key="9" prop="order_count" label="支付订单数" min-width="90" />
        <el-table-column key="8" prop="sum_pay_price" label="支付金额" min-width="90" />
        <el-table-column key="7" prop="create_time" label="最近参与时间" min-width="90" />
      </el-table>
      <el-table
        v-if="type=='order'"
        v-loading="listLoading"
        ...
        size="small"
      >
        <el-table-column key="1" prop="order_sn" label="订单号" min-width="100" />
        ...
        <el-table-column key="4" prop="pay_price" label="订单支付金额" min-width="90" />
        <el-table-column key="5" prop="total_num" label="订单商品数" min-width="90" />
        <el-table-column prop="create_time" label="下单时间" min-width="100" />
        <el-table-column key="6" prop="pay_time" label="支付时间" min-width="100" />
      </el-table>
      <el-table
        v-show="type=='product'"
        ref="tableList"
      ...
      >
        ...
        <el-table-column key="15" prop="mer_name" label="商户名称" min-width="80" />
        ...
        <el-table-column key="19" prop="sales" label="秒杀销量" min-width="70" />
...
// +----------------------------------------------------------------------
import { seckillCardApi,seckillIntervalListApi,seckillParticipantList,seckillOrderList,seckillProductList } from "@/api/marketing";
...
export default {
  ...
  methods: {
    ...
    getAttrValue(row) {
      const _this = this;
      row.map((item) => {
        _this.$set(item, 'mer_name', item.merchant&&item.merchant.mer_name ? item.merchant.mer_name : "");
        _this.$set(item, 'seckill_time_text_arr', (item.seckillActive&&item.seckillActive.seckill_time_text_arr) || []);

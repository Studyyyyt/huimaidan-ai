  ...
  <div class="divBox">
    ...
    <el-card class="mt14">
      <el-table v-loading="loading" :data="tableData.data" size="small">
        <el-table-column label="配送人员" min-width="160">
        ...
        <el-table-column prop="serviceFee" label="配送费" min-width="100" />
        <el-table-column prop="pendingNum" label="待配送" min-width="90" />
    ...
    >
      <div class="drawer-box">
        ...
        <div class="drawer-summary">
          ...
          <div class="summary-item">
            <div class="summary-label">待配送</div>
          ...
          <div class="summary-item">
            <div class="summary-label">配送商品数量</div>
        ...
        <el-table :data="productList" size="small" class="mt14">
          ...
          <el-table-column prop="create_time" label="配送时间" min-width="160" />
          ...
          <el-table-column prop="service_fee" label="配送费" min-width="90" />
          ...
          <el-table-column label="订单状态" min-width="100">
            <template slot-scope="scope">
              <el-tag size="medium" effect="plain" v-show="scope.row.status == 3">配送中</el-tag>
              <el-tag type="success" effect="plain" size="medium" v-show="scope.row.status == 4">已完成</el-tag>
            </template>
          </el-table-column>
          <el-table-column prop="station_name" label="配送门店" min-width="100" />
        </el-table>
      </div>
...
// +----------------------------------------------------------------------
import { deliveryStatisticsList, deliveryStatisticsDetail } from "@/api/systemForm";
...
export default {
  ...
  methods: {
    ...
    onchangeTime(e) {
      this.timeVal = e || [];
      this.tableFrom.delivery_date = this.timeVal && this.timeVal.length === 2 ? this.timeVal : [];
      this.getList(1);
    },
    // 本地列表筛选 + 分页
    getList(num) {
      ...
      deliveryStatisticsList(this.tableFrom).then(res => {
    ...
    // 详情展示（占位）
    onDetail(row) {
      deliveryStatisticsDetail(row.service_id).then(res=>{
        this.detailData = res.data;
          this.drawer = true;
          const list = (res.data.deliveryOrder || []).reduce((acc, order) => {
            const items = order.storeOrder.orderProduct || [];
            items.forEach(op => {
              const product = op;
              this.$set(product,'order_sn', order.storeOrder.order_sn);
              this.$set(product,'pay_price', order.storeOrder.pay_price);
              this.$set(product,'service_fee', order.service_fee);
              this.$set(product,'status', order.status);
              this.$set(product,'count', order.storeOrder.orderProduct[0].product_num);
              this.$set(product,'create_time', order.create_time);
              this.$set(product,'station_name', order.station ? order.station.station_name : '');
              if (product) acc.push(product);
            });
            return acc;
        }, []);
        this.productList = list;
        this.detailData.productCount = res.data.deliveryOrder.reduce((acc, order) => {
          return acc + order.storeOrder.orderProduct.reduce((acc, product) => {
            return acc + product.product_num;
          }, 0);
        }, 0);
        }).catch(res=>{
        this.$message.error(res.msg);

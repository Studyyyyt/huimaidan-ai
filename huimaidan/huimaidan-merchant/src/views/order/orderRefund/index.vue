          ...
          <el-form :model="tableFrom" ref="searchForm" size="small" label-width="85px" inline>
            <el-form-item label="订单状态：" style="display: block;" prop="status">
              <el-radio-group v-model="tableFrom.status" type="button" @change="getList(1)">
                ...
                <el-radio-button
                  label="-2"
                >已取消 {{ '(' +orderChartType.cancelled?orderChartType.cancelled:0+ ')' }}</el-radio-button>
              </el-radio-group>
            </el-form-item>
        ...
        >
          ...
          <el-table-column label="退款单号" min-width="100">
            <template slot-scope="scope">
              ...
              <span v-if="scope.row.status == 4" style="color: #ED4014;display: block;">平台介入</span>
            </template>
          </el-table-column>
          <el-table-column prop="user.nickname" label="用户信息" min-width="100" />
          <el-table-column prop="refund_price" label="退款金额" min-width="80" />
          <el-table-column prop="nickname" label="商品信息" min-width="250">
            <template slot-scope="scope">
              <div
          ...
          <el-table-column label="操作" min-width="150" fixed="right">
            <template slot-scope="scope">
              <el-button type="text" size="small" @click="onOrderDetail(scope.row.order.order_id)">订单详情</el-button>
              <el-button type="text" size="small" @click="onRefundOrderDetail(scope.row.refund_order_id)">退款单详情</el-button>
              <el-button v-if="scope.row.status === 0" type="text" size="small" @click="onOrderStatus(scope.row.refund_order_id)">退款审核</el-button>
              <!-- <el-button type="text" size="small" @click="onOrderLog(scope.row.refund_order_id)">订单记录</el-button> -->
              <el-button v-if="scope.row.status === 2" type="text" size="small" @click="confirmReceipt(scope.row)">退货审核</el-button>

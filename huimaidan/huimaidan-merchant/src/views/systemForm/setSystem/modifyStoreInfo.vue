<template>
  <div class="information">
    ...
    <div v-if="merModel" class="business-msg" style="min-height: 600px;">
      <div v-if="infoType == '1'" class="user-msg">
        <div class="basic-information">
          <span class="basic-label">店铺名称：</span>
          {{ merData.mer_name }}
        </div>
        <div class="basic-information">
          <span class="basic-label">店铺负责人手机号：</span>
          {{ merData.mer_phone }}
        </div>
        <div
          v-if="merData.merchantCategory.category_name"
          class="basic-information"
        >
          <span class="basic-label">店铺分类：</span>
        ...
        <div
          ...
          class="basic-information"
        >
          <span class="basic-label"> 店铺类别：</span>
          {{ merData.is_trader ? "自营" : "非自营" }}
        </div>
        <div class="basic-information">
          <span class="basic-label"> 店铺负责人姓名：</span>
          {{ merData.real_name }}
        </div>
        <div class="basic-information">
          <span class="basic-label"> 店铺入驻时间：</span>
          {{ merData.create_time }}
        </div>
        ...
        <div
          v-if="merData.sub_mchid && merData.merchantType"
          class="basic-information"
        >
          <span class="basic-label"> 店铺类型：</span>
          {{ merData.merchantType.type_name }}
        ...
        <div class="basic-information">
          <div>
            <span class="basic-label">是否开启店铺：</span>
            <el-tooltip
              v-if="merData.is_margin == 1 && merData.mer_state == 0"
              class="item"
              effect="dark"
              content="请先支付店铺保证金！"
              placement="top-start"
          ...
          <div v-if="merData.is_margin != 0">
            ...
            <div v-if="merData.is_margin == 1">
              ...
              <div class="margin_count" @mouseenter="getCode()">
                <el-button type="text" size="small" class="mr10 pay_btn"
                  >去支付保证金</el-button
                >
                <!--支付二维码-->
                <div class="erweima">
            ...
            <div class="margin_main" v-if="merData.is_margin == 10">
              ...
              <div class="margin_count">
                <span class="mr10 spanBtn" @click="viewRecords"
                  >查看保证金记录</span
                >
                <!--保证金弹窗-->
                <div class="margin_modal" @mouseleave="supplyPay = false">
                  <div>
                    ...
                    <div class="alic">
                      <span class="text_g"
                        >剩余保证金￥{{ merData.margin }}元</span
                      >
                      <el-button
                        v-if="merData.margin > 0"
                        type="primary"
                        size="small"
                        @click="applyReturn"
                        >申请退回保证金</el-button
            ...
            <div
              class="margin_main"
              v-if="merData.is_margin == -10 || merData.is_margin == -1"
            >
              <span class="basic-label">店铺保证金：</span>
              <span class="margin_price">{{ merData.margin }}元</span>
              <div class="margin_count">
                ...
                <div class="margin_modal" @mouseleave="goPay = false">
                  <div>
                    <img
                      v-if="merData.is_margin == -10"
                      src="@/assets/images/margin01.png"
                    />
                    <img
                      v-if="merData.is_margin == -1"
                      src="@/assets/images/margin02.png"
                    />
                    <img
                      v-if="merData.is_margin == 10"
                      src="@/assets/images/margin03.png"
                    />
                    ...
                    <div class="alic" v-if="merData.is_margin == -10">
                      ...
                      <div class="margin_refused">
                        未通过原因：<span>{{
                          merData.refundMarginOrder.refusal
                        }}</span>
                      </div>
                      <el-button
                        type="primary"
      ...
      >
        <div class="bank-container">
          <div class="item">
            说明：
            <span class="red">申请退回保证金则视为关闭店铺，请谨慎操作！</span>
          </div>
          <div class="item">
            <div v-if="online > 0">
              线上支付的保证金 <span class="red">{{ online }}元</span>会原路返回
            </div>

            <div v-if="offline > 0">
              线下支付的保证金<span class="red">{{ offline }}元，</span
              >会通过下方账号信息返回，请务必确认下方您的收款信息真实有效，以确保资金可顺利退回，感谢配合！
            </div>
          </div>
          ...
          >
            ...
            <el-form-item label="开户银行：" prop="code">
              <el-input
                v-model="bankValidate.code"
                type="text"
                placeholder="请输入开户银行"
              />
            </el-form-item>
            <el-form-item
              v-if="bankValidate.type == 1"
              label="银行卡号："
              prop="pic"
            >
              <el-input
                v-model="bankValidate.pic"
                type="number"
                placeholder="请输入银行卡号"
              />
            </el-form-item>
            <el-form-item v-if="bankValidate.type == 2" label="收款二维码：">
              <img
                v-if="bankValidate.pic"
                :src="bankValidate.pic"
        ...
        <span slot="footer" class="dialog-footer">
          <el-button
            type="primary"
            size="small"
            @click="bankConfirm('bankValidate')"
            >确定</el-button
          >
        </span>
      </el-dialog>
        ...
        <div class="form-data">
          ...
          >
            <el-form-item
              ...
              prop="mer_banner"
            >
              <div class="upLoadPicBox" @click.stop="modalPicTap('1')">
                <div v-if="merData.mer_banner" class="pictrue">
                  <el-image ref="merBanner" :src="merData.mer_banner"/>
                  <!-- <div class="banner-mask">
                    <i
                      class="el-icon-delete"
                      @click.stop="merData.mer_banner=''"
                    />
                    <i class="el-icon-view" @click.stop="viewabanner('merBanner')" />
                  </div> -->
                </div>
            ...
            <el-form-item
              ...
              prop="mer_avatar"
            >
              <div class="upLoadPicBox" @click.stop="modalPicTap('2')">
                <div v-if="merData.mer_avatar" class="pictrue">
                  <el-image :src="merData.mer_avatar" />
                  <!-- <div class="banner-mask">
                    <i
                      class="el-icon-delete"
                      @click.stop="merData.mer_avatar=''"
                    />
                    <i class="el-icon-view" @click.stop="viewabanner(merData.mer_avatar)" />
                  </div> -->
                </div>
                <div v-else class="upLoad">
                  <i class="el-icon-camera cameraIconfont" />
            ...
            <el-form-item class="form-item" label="店铺街背景图：">
              <div class="upLoadPicBox" @click.stop="modalPicTap('3')">
                <div v-if="merData.mini_banner" class="pictrue">
                  <el-image :src="merData.mini_banner" />
                  <!-- <div class="banner-mask">
                    <i
                      class="el-icon-delete"
                      @click.stop="merData.mini_banner=''"
                    />
                    <i class="el-icon-view" @click.stop="viewabanner(merData.mini_banner)" />
                  </div> -->
                </div>
                <div v-else class="upLoad">
                  <i class="el-icon-camera cameraIconfont" />
            ...
            >
              <div class="upLoadPicBox_qualification">
                <div
                  v-for="(item, index) in uploadedQualifications"
                  :key="index"
                  class="uploadpicBox_list"
                >
                  ...
                  <div class="uploadpicBox_list_method">
                    <i
                      class="el-icon-delete"
                      @click="deldetQualificationsList(index)"
                    />
            ...
            <el-form-item label="配送方式:" prop="delivery_way">
              ...
              <span class="trip"
                >只选择一种配送方式时,会自动修改店铺所有商品的配送方式</span>
            </el-form-item>
            <!-- <el-row
              v-if="
            ...
            >
              <el-col :span="24">
                <el-form-item label="详细地址：" prop="mer_take_address">
                  <el-input
                    v-model="merData.mer_take_address"
                    placeholder="请输入详细地址"
                  >
                    <el-button slot="append" @click="onSearchs"
                      >查找位置</el-button
                    >
                  </el-input>
                </el-form-item>
            ...
            <el-row>
              <el-col :span="24">
                <el-form-item label="店铺简介：" prop="mer_info">
                  <el-input
                    v-model="merData.mer_info"
                    type="textarea"
              ...
              <el-col :span="24">
                <el-form-item label="店铺地址：" prop="mer_address">
                  <el-input
                    ...
                    enter-button="查找位置"
                    placeholder="请输入店铺地址（地址中请包含城市名称，否则会影响搜索精度）"
                  >
                  ...
                  <div class="trip">
                    <template v-if="merData.lat && merData.long">
                      当前经纬度：{{ merData.lat }},{{ merData.long }}
                    </template>
                    <template v-else>
                      请点击查找位置获取经纬度
                    </template>
      ...
      <div v-if="infoType == '3'" class="user-msg">
        <div class="basic-information">
          <span class="basic-label"> 店铺手续费：</span>
          {{
            Number(merData.commission_rate) > 0
              ? parseFloat(merData.commission_rate).toFixed(2)
              : parseFloat(
                  merData.merchantCategory.commission_rate * 100
                ).toFixed(2)
          }}%
        </div>
        <div class="basic-information">
        ...
        <div class="basic-information">
          <span class="basic-label"> 平台首页推荐店铺：</span>
          {{ merData.is_best ? "是" : "否" }}
        </div>
        <div class="basic-information">
          <span class="basic-label"> 线下支付：</span>
          {{ merData.offline_switch==1 ? "开启" : "关闭" }}
        </div>
      </div>
    ...
    <el-dialog
      v-if="currImgDialog"
      :visible.sync="currImgDialog"
      width="400px"
      close-on-click-modal
      class="mapBox"
      custom-class="dialog-scustom"
    >
    <el-image :src="currentImage" style="width: 100%;"></el-image>
    </el-dialog>
  </div>
</template>
...
import { DELIVERY_TYPE_LIST } from './domain/delivery.props';
export default {
  name: "Information",
  ...
  data() {
    var checkPhone = (rule, value, callback) => {
      if (value === "") {
        ...
        if (!regPone.test(value)) {
          return callback(
            new Error(
              "请填写客服电话(座机格式'区号-座机号码'),400格式：400-XXX-XXXX"
            )
          );
        }
        callback();
    ...
    return {
      ...
      deliveryList: DELIVERY_TYPE_LIST,
      rules: {
        ...
        mer_keyword: [
          { required: false, message: "请输入店铺关键字", trigger: "blur" }
        ],
        mer_address: [
        ...
        mer_take_time: [
          { required: true, message: "请选择提货点营业时间", trigger: "change" }
        ],
        mer_take_phone: [
  ...
  watch: {
    ...
    "merData.delivery_way": {
      handler(val, oldVal) {
        if (val.includes("1")) {
          if (!this.merData.mer_take_address) {
            this.merData.mer_take_address = this.merData.mer_address;
          }
        }
      },
      deep: true
  ...
  methods: {
    ...
    selectAdderss(data) {
      ...
      this.merData.lat = data.latlng.lat;
      this.merData.long = data.latlng.lng;
      this.modalMap = false;
    },
    onSearchs() {
    ...
    getInfo() {
      ...
      getBaseInfo().then((res) => {
        ...
        that.keyUrl = `https://apis.map.qq.com/tools/locpicker?type=1&key=${keys}&addr=${res.data.mer_take_address}&referer=myapp`
        const info = res.data || null
        if (info.mer_take_time) {
        ...
        that.merData.mer_take_address = info.mer_take_address ? info.mer_take_address : info.mer_address
    ...
    submitForm(formName) {
      if (this.infoType == 2) {
        if (!this.merData.lat || !this.merData.long) {
          this.$message.error("请通过查找位置获取经纬度后再进行提交");
          return false;
        }
    ...
    getRecordList() {
      let that = this;
      that.loading = true;
      marginRecordLst(that.tableFrom)
        .then(async res => {
    ...
    applyReturn() {
      ...
      that
        .$confirm(
          "申请退回保证金则视为关闭店铺，请谨慎操作！您是否确定继续操作？",
          "提示",
          {
    ...
    // 查找位置
    onSearch() {
      this.address = this.merData.mer_address
    },
    // 获取 map key
    getMapInfo() {
      const that = this;
    ...
    viewabanner(){
      ...
      this.$refs.merBanner.clickHandler();
    }
  }
};
</script>
...
.banner-mask, .uploadpicBox_list_method{
  position: absolute;
  top: 0;
  left: 0;
  font-size: 18px;

  ...
  <div class="label-wrapper">
    ...
    <template v-else>
      <div class="label-box" v-for="(item, index) in labelList" :key="index">
        ...
        <div class="list">
          <div class="label" :class="{ on: label.disabled }" v-for="(label, j) in item.list" :key="j" v-db-click
  ...
  methods: {
    ...
    storeLabel(data) {
      ...
      productLabelUseListApi()
        .then((res) => {
          const labelGroups = res.data && res.data.length && res.data[0].list
            ? res.data
            : [{ cate_name: '商品标签', list: res.data || [] }];
          labelGroups.map((el) => {

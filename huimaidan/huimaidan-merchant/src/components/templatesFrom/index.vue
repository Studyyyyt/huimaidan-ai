<template>
  <el-dialog title="运费模板" center :visible.sync="dialogVisible" width="1060px">
    <div>
      <el-form ref="ruleForm" v-loading="loading" :model="ruleForm" label-width="130px" size="small" :rules="rules">
        ...
        <el-form-item label="配送区域及运费：" prop="region">
          <el-table v-loading="listLoading" :data="ruleForm.region" border fit highlight-current-row size="small" class="tempBox">
            ...
            <el-table-column width="120" align="center" :label="columns.title">
              <template slot-scope="{row}">
                <el-input-number v-model="row.first" controls-position="right" :min="0" />
              </template>
            </el-table-column>
            <el-table-column width="120px" align="center" label="运费（元）">
              <template slot-scope="{row}">
                <el-input-number v-model="row.first_price" controls-position="right" :min="0" />
              </template>
            </el-table-column>
            <el-table-column width="120px" align="center" :label="columns.title2">
              <template slot-scope="{row}">
                <el-input-number v-model="row.continue" controls-position="right" :min="0.1" />
              </template>
            </el-table-column>
            <el-table-column class-name="status-col" align="center" label="续费（元）" width="120">
              <template slot-scope="{row}">
...
export default {
  ...
  mounted() {
    // this.getCityList()
    if (this.tempId > 0) this.getInfo(this.tempId)
  },
  methods: {
    resetForm(formName) {
      // this.$msgbox.close()
    ...
    getInfo(id) {
      ...
      templateDetailApi(id).then(res => {
        ...
        this.ruleForm = {
          ...
          city_id3: info.undelives&&info.undelives.city_ids || []
        }
...
.tempBox ::v-deep .el-input-number--small{
  width: 100px !important;
}

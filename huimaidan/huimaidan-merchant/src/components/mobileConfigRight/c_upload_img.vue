<template>
  <div class="upload_img" :class="configData.type == 'code' ? 'on' : ''" v-if="configData">
    ...
    <div class="list">
        ...
        <div class="picTxt">
          <div class="box" @click="modalPicTap('单选')">
            <div class="pictrue acea-row row-center-wrapper" v-if="configData.url">
              ...
              <div v-if="configData.delType" class="iconfont iconshanchu-01" @click.stop="bindDelete"></div>
            </div>
      ...
      <div v-if="configData.type != 'code' && defaults.name == 'customerService' && defaults.buttonConfig.tabVal == 0"
        class="item">
    ...
    <div>
      <el-dialog :visible.sync="modalPic" width="960px" :title="configData.name ? configData.name : '上传图片'">
        <uploadPictures :isChoice="isChoice" @getPic="getPic" :gridBtn="gridBtn" :gridPic="gridPic"
          :isType="configData.video ? 2 : 1" v-if="modalPic"></uploadPictures>
      </el-dialog>
    </div>
    ...
    <el-dialog :visible.sync="modalVideo" width="480px" title="上传视频">
      <uploadVideo @getVideo="getvideo" v-if="modalVideo" :isDiy="true"></uploadVideo>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getPic(pc) {
      console.log(pc);
      this.$nextTick(() => {
        this.configData.url = pc.att_dir[0];
        this.modalPic = false;
      });
    },
  },

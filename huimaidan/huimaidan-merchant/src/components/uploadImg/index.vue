            ...
            <div class="acea-row">
              <div class="uploadCont">
                ...
                >
                  ...
                  <div
                    v-if="['png','jpg','gif','jepg','svg','avif'].indexOf(file.name.substr(file.name.lastIndexOf('.')+1)) != -1"
                    slot="file"
                    slot-scope="{ file }"
                    draggable="false"
...
export default {
  ...
  methods: {
    ...
    async fileChange(file, fileList) {
      if(['image/jpeg','image/png','image/jpg','image/gif','image/webp','image/avif'].indexOf(file.raw.type) == -1){
        // 过滤出有效的图片文件
        let validFiles = this.ruleForm.imgList.filter(item => {
          const isImage = item.raw && item.raw.type.startsWith('image/');
          const isValidType = ['image/jpeg','image/png','image/jpg','image/gif','image/webp'].includes(item.raw.type);
          return isImage && isValidType;
        });
        // 更新文件列表，只保留有效文件
        this.ruleForm.imgList = validFiles;

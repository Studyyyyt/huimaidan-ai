...
class ConfigValue extends BaseController
{
    ...
    public function save($key)
    {
        ...
        if($key == 'circle_config') {
            $one = (float)$formData['one_agent_commission'];
            $two = (float)$formData['two_agent_commission'];
            $three = (float)$formData['three_agent_commission'];
            if (!($one >= 0 && $one <= 100 && 
                $two >= 0 && $two <= 100 && 
                $three >= 0 && $three <= 100)) {
                return app('json')->fail('佣金比例必须在0到100之间');
            }
            if (!($one >= $two && $two >= $three)) {
                return app('json')->fail('佣金比例必须逐级递减');
            }
        }

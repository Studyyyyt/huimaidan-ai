...
class ConfigOthers extends BaseController
{
    ...
    public function update()
    {
        $data = $this->request->params([
            'extension_status',
            // [已禁用二级分销] 原接收 'extension_two_rate' 参数，现仅保留一级分销
            'extension_one_rate',
            'extension_self',
            'extension_limit',
            'extension_limit_day',
        ...
        // [已禁用二级分销] 原校验二级比例，现强制二级比例为0，仅允许一级分销
        $data['extension_two_rate'] = 0;
        if ($data['extension_one_rate'] < 0)
            return app('json')->fail('比例不能小于0');
        // [已禁用二级分销] 原校验一级比例不能小于二级比例、比例之和不能超过100%，现仅校验一级
        if (bccomp($data['extension_one_rate'], 1, 3) == 1)
            return app('json')->fail('一级比例不能超过1，即100%');

...
class Merchant extends BaseController
{
    ...
    {
        $where = $this->request->params(['keyword', 'date', 'status', 'statusTag', 'is_trader', 'category_id', 'city_id', 'type_id', 'is_best','offline_switch','region_id']);
    ...
    public function lst()
    {
        ...
        $where = $this->request->params(['keyword', 'date', 'status', 'statusTag', 'is_trader', 'category_id', 'city_id', 'type_id', ['order', 'create_time'], 'is_best','offline_switch','region_id', 'business_id']);
    ...
    public function update($id, MerchantValidate $validate, MerchantCategoryRepository $merchantCategoryRepository)
    {
        ...
        if (!empty($data['city_id']) && !$this->repository->cityExists((int)$data['city_id']))
            return app('json')->fail('所属城市不存在');
        ...
        Db::transaction(function () use ($id, $data) {
            ...
            // 提取 configured_sales，保存到 merchant_profile 表
            $configuredSales = $data['configured_sales'] ?? 0;
            unset($data['configured_sales']);
            ...
            // 保存配置销量到 merchant_profile 表
            /** @var MerchantProfileRepository $profileRepository */
            $profileRepository = app()->make(MerchantProfileRepository::class);
            $profileRepository->saveByMerId((int)$id, ['configured_sales' => $configuredSales]);
        });
    ...
    {
        $data = $this->request->params([['category_id', 0], ['city_id', 0], ['type_id', 0], 'mer_name', 'commission_rate', 'real_name', 'mer_phone', 'mer_keyword', 'mer_address', ['long', ''], ['lat', ''], 'mark', ['sort', 0], ['status', 1], ['mer_state', 1], ['is_audit', 0], ['is_best', 0], ['is_bro_goods', 0], ['is_bro_room', 0], ['is_trader', 0], 'sub_mchid', ['commission_switch', 0],['offline_switch',0],['region_id', 0],['business_id', 0],['applyment_id',''],['store_group_ids', []], ['huimaidan_settlement_mode', 1], ['huimaidan_withdraw_rate', 0], ['huimaidan_min_extract_money', 500], ['configured_sales', 0]]);
        $data = array_merge($data, $this->repository->normalizeHuimaidanSettlementConfig($data));
        if (!$isUpdate) {

...
class Menu extends BaseController
{
    ...
    public function menus()
    {
        ...
        foreach ($menus as $k => $menu) {
            // 独立 HTML 管理页面使用绝对路径，避免被追加 /admin 前缀导致 SPA 路由 404
            if (substr($menu['path'], -5) === '.html') {
                if (substr($menu['path'], 0, 1) !== '/') {
                    $menu['path'] = '/' . $menu['path'];
                }
            } else {
        ...
        // 过滤掉父菜单不存在的子菜单（当父菜单被隐藏时，子菜单也不应显示）
        $menuIds = array_column($menus, 'id');
        $menus = array_filter($menus, function ($v) use ($menuIds) {
            return $v['pid'] == 0 || in_array($v['pid'], $menuIds);
        });

...
//            return app('json')->fail('商品有被加入购物车不可删除');
    ...
    {
        // [已禁用二级分销] 原获取 extension_two_rate，现不再获取
        $data = systemConfig(['extension_status', 'svip_switch_status', 'integral_status', 'extension_one_rate']);
        ...
        // [已禁用二级分销] 原设置分销二比例，现强制为0
        $data['extension_two_rate'] = 0;
    ...
    {
        $ids = $this->request->param('ids');
        // [已禁用二级分销] 原获取 extension_two，现不再获取
        $data = $this->request->params(['extension_one']);
        $data['extension_two'] = 0;
        if ($data['extension_one'] > 1 || $data['extension_one'] < 0) {

...
class RefundOrder extends BaseController
{
    ...
    public function check($id, StoreOrderRepository $storeOrderRepository)
    {
        ...
        return app('json')->success(compact('activity_type', 'total_refund_price', 'products', 'status'));
    }
...
//        if (!$order->refund_status) return app('json')->fail('订单已过退款/退货期限');
    ...
    {
        $data = $this->request->params(['refund_message', 'mer_mark']);
        $orderId = $this->request->param('order_id', 0);
        ...
        $refund = $this->repository->merchantOrderRefund($order, $data);

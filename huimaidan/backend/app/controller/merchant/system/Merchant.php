...
class Merchant extends BaseController
{
    ...
    public function updateForm()
    {
        ...
        $formData = array_merge(
            $this->request->merchant()->toArray(),
            app()->make(MerchantProfileRepository::class)->formData($this->request->merId())
        );
        return app('json')->success(formToData($this->repository->merchantForm($formData)));
    }
    ...
    public function update(MerchantUpdateValidate $validate, MerchantTakeValidate $takeValidate, MerchantTakeRepository $repository)
    {
        ...
        if ($type == 2) {
            ...
            $profileFields = [
                'branch_name', 'configured_sales', 'per_capita', 'business_hours',
                'has_large_table', 'has_baby_chair', 'has_private_room', 'can_phone_reserve', 'is_non_smoking',
                'promo_image', 'slogan',
            ];
        ...
        if (is_null($profileData)) {
            ...
            Db::transaction(function () use ($merchant, $data, $profileData) {
                $merchant->save($data);
                app()->make(MerchantProfileRepository::class)->saveByMerId((int)$merchant->mer_id, $profileData);
            });
        }
    ...
    public function info(MerchantTakeRepository $repository)
    {
        ...
        $data['huimaidan_profile'] = app()->make(MerchantProfileRepository::class)->formData((int)$merchant->mer_id);
        $data = array_merge($data, $data['huimaidan_profile']);

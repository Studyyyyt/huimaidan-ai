...
class UserRelation extends BaseController
{
    ...
    public function merchantList()
    {
        ...
        $params = $this->request->params(['store_group_id', 'order', 'latitude', 'longitude', 'distance']);
        if ($params['store_group_id'] !== '') {
            $where['store_group_id'] = $params['store_group_id'];
        }
        // 位置筛选（对齐首页 store/list）：distance 距离筛选、order=location 按距离排序
        foreach (['order', 'latitude', 'longitude', 'distance'] as $field) {
            if ($params[$field] !== '') {
    ...
    public function checkCollection($id)
    {
        $id = (int)$id;
        if ($id <= 0) {
            return app('json')->fail('请选择商户');
        }
        return app('json')->success($this->repository->merchantCollectionCheck($this->request->uid(), $id));
    }
    ...
    {
        $ids = $this->request->param('type_id/a');

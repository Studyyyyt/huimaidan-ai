...
//        }
    ...
    /**
     * 小程序单 token 无感续期。
     * 复用 UserTokenMiddleware 完成 token 解析、过期宽限和用户状态校验。
     */
    public function refreshToken(UserRepository $repository)
    {
...
//            if (!$userInfo) throw new ValidateException('openid获取失败');
    ...
    public function authLogin()
    {
        ...
        $isRoutineAuth = ($auth['type'] ?? '') === 'routine';
        $phoneBindingPolicy = app()->make(RoutinePhoneBindingPolicy::class);
        $users = $this->authInfo($auth, !$isRoutineAuth && !systemConfig('is_phone_login'));
        if (!$users)
            return app('json')->fail('授权失败');
        $authInfo = $users[0];
        $userRepository = app()->make(UserRepository::class);
        ...
        if ($isRoutineAuth && $phoneBindingPolicy->requiresBinding($user)) {
            return app('json')->status(201, $this->createRoutinePhoneAuth($authInfo, (int)($auth['auth']['spread'] ?? 0), $phoneBindingPolicy));
        }
        if (!$user) {
            return app('json')->status(201, $this->createRoutinePhoneAuth($authInfo, (int)($auth['auth']['spread'] ?? 0), $phoneBindingPolicy));
        }
    ...
    private function createRoutinePhoneAuth($authInfo, int $spread, RoutinePhoneBindingPolicy $phoneBindingPolicy): array
    {
        $uni = uniqid(true, false) . random_int(1, 100000000);
        $key = 'U' . md5(time() . $uni);
        Cache::set('u_try' . $key, ['id' => $authInfo['wechat_user_id'], 'type' => $authInfo['user_type'], 'spread' => $spread], 3600);

        return [
            'key' => $key,
            'wechat_phone_switch' => $phoneBindingPolicy->switchValue(),
        ];
    }
    ...
    {
        $code = $this->request->param('code/s');
        $phoneCode = $this->request->param('phone_code/s', '');
        $auth_token = $this->request->param('auth_token/s');
        $iv = $this->request->param('iv');
        $encryptedData = $this->request->param('encryptedData/s');
        $miniProgramService = MiniProgramService::create();
        if (!$phoneCode && $code && !$iv && !$encryptedData) {
            $phoneCode = $code;
        }

        if ($phoneCode) {
            $data = $miniProgramService->getPhoneNumberByCode($phoneCode);
        } else {
            $userInfoCong = Cache::get('eb_api_code_' . $code);
            if (!$code && !$userInfoCong)
                throw new ValidateException('授权失败,参数有误');

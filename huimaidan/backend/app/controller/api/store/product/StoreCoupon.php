...
class StoreCoupon extends BaseController
{
    ...
    public function usable(StoreCouponUserRepository $repository)
    {
        [$page, $limit] = $this->getPage();
        $merId = (int)$this->request->param('shopId', $this->request->param('mer_id', 0));
        $amount = $this->request->param('amount', '');
        if ($merId <= 0) {
            return app('json')->fail('请选择商户');
        }
        if ($amount === '' || !is_numeric($amount) || (float)$amount <= 0) {
            return app('json')->fail('请输入正确的订单金额');
        }
        return app('json')->success($repository->usableForHuimaidan($this->request->uid(), $merId, $amount, $page, $limit));
    }
}

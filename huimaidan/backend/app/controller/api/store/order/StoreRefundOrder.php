...
class StoreRefundOrder extends BaseController
{
    ...
    {
        $_id  = (string)$this->request->param('ids/s', '');
        $type = (string)$this->request->param('type/s', '');
        $ids  = explode(',', $_id);
        if (!$_id || !count($ids))
    ...
    {
        $refund = $this->request->param('refund/a', []);
...
//        if (!$order->refund_status) return app('json')->fail('订单已过退款/退货期限');
     ...
     * 订单全额退款申请
     * @param int $id 订单ID
     * @param StoreRefundOrderValidate $validate 验证器
     * @param StoreOrderRepository $orderRepository 订单仓库
     * @return \think\response\Json
     * @throws \think\db\exception\DataNotFoundException
     * @throws \think\db\exception\DbException
     * @throws \think\db\exception\ModelNotFoundException
    ...
    {
        $data = $this->request->params(['order_id', 'refund_message', 'mark', 'pics']);
        $validate->check($data);
        ...
        if ($order->order_type == 2 && (isset($order->deliveryOrder) && in_array($order->deliveryOrder->status, [3,4])))
            return app('json')->fail('订单已配送，不支持退款');
        if (!$order->refund_status)
            return app('json')->fail('订单已过退款期限');
        if ($order->status < 0) return app('json')->fail('订单已退款');
        if ($order->status == 10) return app('json')->fail('订单不支持退款');
        if ($order->paid != 1) return app('json')->fail('订单未支付');

        $refund = $this->repository->orderRefund($order, $uid, $data);

        return app('json')->success('提交申请成功', ['refund_order_id' => $refund->refund_order_id]);
    }

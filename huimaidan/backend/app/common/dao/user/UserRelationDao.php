...
class UserRelationDao extends BaseDao
{
    ...
    public function search($where)
    {
        ...
        $query = ($this->getModel()::getDB())
            ...
            ->when((isset($where['type_ids']) && is_array($where['type_ids'])), function ($query) use ($where) {
                // type_ids 仅用于收藏商户列表筛选（type=10）。JOIN merchant 表后，
                // merchant 表也存在 type_id 列，不带表前缀会触发 "Column ambiguous"，
                // 故显式限定为收藏关系表 user_relation.type_id
                $relationTable = model::getDB()->getTable();
                $query->whereIn($relationTable . '.type_id', $where['type_ids']);
            });

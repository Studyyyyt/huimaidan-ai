...
class FinancialDao extends BaseDao
{
    ...
    public function search(array $where)
    {
        ...
        $query->when(isset($where['status']) && $where['status'] !=='',function($query) use($where){
            ...
            ->when(isset($where['business_type']) && $where['business_type'] !=='',function($query) use($where){
                $query->where('Financial.business_type',$where['business_type']);
            })
            ->when(isset($where['trade_channel']) && $where['trade_channel'] !=='',function($query) use($where){
                $query->where('Financial.trade_channel',$where['trade_channel']);
            })
            ->when(isset($where['account_type']) && $where['account_type'] !=='',function($query) use($where){
                $query->where('Financial.account_type',$where['account_type']);
            })

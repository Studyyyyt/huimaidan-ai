    ...
    public function cartIbByData(array $ids, int $uid, ?UserAddress $address)
    {
        ...
        return StoreCart::getDb()->where('uid', $uid)->with([
            ...
            'productAttr' => function (Relation $query) {
                // [已禁用二级分销] 原append bc_extension_two，现移除
                $query->field('value_id,image,extension_one,extension_two,product_id,stock,price,unique,sku,volume,weight,ot_price,cost,svip_price,library_id,bar_code,bar_code_number,settlement_price')->with(['product'])->append(['bc_extension_one']);
                // $query->field('value_id,image,extension_one,extension_two,product_id,stock,price,unique,sku,volume,weight,ot_price,cost,svip_price,library_id,bar_code,bar_code_number,settlement_price')->with(['product'])->append(['bc_extension_one', 'bc_extension_two']);
            },
            'reservation',
            'reservationAttr'

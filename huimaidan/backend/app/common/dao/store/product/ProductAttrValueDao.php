...
class ProductAttrValueDao extends BaseDao
{
    ...
    public function add($data,$isType)
    {
        switch ($isType) {
            ...
            case 4: // 预约商品
                $reservation = [];
                foreach ($data['reservation'] as $val) {
                    $item = [];
                ...
                $this->createReservation($sku, $reservation);
                break;
            default:
                $this->insertAll($data);
                break;

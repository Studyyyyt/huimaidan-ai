...
class UserTokenMiddleware extends BaseMiddleware
{
    ...
    public function before(Request $request)
    {
        ...
        try {
            $token = $this->tokenFromHeaders([
                'X-Token' => $request->header('X-Token'),
                'Authorization' => $request->header('Authorization'),
                'Authori-zation' => $request->header('Authori-zation'),
            ]);
            if (!$token)
    ...
    protected function tokenFromHeaders(array $headers): string
    {
        foreach (['X-Token', 'Authorization', 'Authori-zation'] as $header) {
            $token = trim((string)($headers[$header] ?? ''));
            if ($token !== '') {
                return $this->withoutBearerPrefix($token);
            }
        }
        return '';
    }

    protected function withoutBearerPrefix(string $token): string
    {

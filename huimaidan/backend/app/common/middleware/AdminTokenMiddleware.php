...
class AdminTokenMiddleware extends BaseMiddleware
{
    ...
    public function before(Request $request)
    {
        ...
        try {
            ...
            if(!$token) $token = trim($request->param('token/s',''));
        ...
        if ($admin->region_ids) {
            if($admin->is_agent == 1) {
                $where['region_id'] = $admin->region_ids;
            }
            if($admin->is_agent == 2) {
                $where['business_id'] = $admin->region_ids;
            }
            $regionMerId = app()->make(MerchantRepository::class)->search($where)->column('mer_id');
        }
        $request->macro('isLogin', function () {
            return true;
        });
        ...
        $request->macro('isAgent', function () use (&$admin) {
            return $admin->is_agent;
        });

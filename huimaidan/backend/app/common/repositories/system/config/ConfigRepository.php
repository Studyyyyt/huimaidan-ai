...
class ConfigRepository extends BaseRepository
{
    ...
    public function getComponent($config, $merId)
    {
        ...
        switch ($config['config_type']) {
            ...
            case 'switches':
                // 创建开关组件，并处理联动效果
                $component = Elm::{$config['config_type']}($config['config_key'], $config['config_name'])->className('switch-width-double')->activeText('开启')->inactiveText('关闭');
    ...
    {
        $this->ensureRoutinePaySandboxConfigs($configClassify, $merId);
        // 根据配置分类ID和商家ID（0代表系统配置）查询配置信息
        $config = $this->dao->cidByConfig($configClassify->config_classify_id, $merId == 0 ? 0 : 1);
        // [已禁用二级分销] 从表单配置中移除 extension_two_rate，使其不在 distribution_tabs 页面显示
        $config = $config->filter(function ($item) {
            return $item['config_key'] !== 'extension_two_rate';
        });
        // 获取查询到的配置项的键名集合
    ...
    protected function ensureRoutinePaySandboxConfigs(SystemConfigClassify $configClassify, int $merId): void
    {
        if ($merId !== 0) {
            return;
        }

        $isRoutinePayClassify = $this->dao->search(['config_classify_id' => $configClassify->config_classify_id])
            ->where('config_key', 'pay_routine_mchid')
            ->count() > 0;

        if (!$isRoutinePayClassify) {
            return;
        }

        $items = [
            'pay_routine_sandbox_enabled' => [
                'config_name' => '小程序支付沙箱模式',
                'config_type' => 'switches',
                'config_rule' => '',
                'config_props' => "defaultValue = '0'",
                'info' => '仅作用于小程序微信支付v2。开启后请求改走微信支付仿真测试地址，失败会直接显式返回错误。',
                'sort' => 69,
            ],
            'pay_routine_sandbox_key' => [
                'config_name' => '小程序支付沙箱验签密钥',
                'config_type' => 'input',
                'config_rule' => '',
                'config_props' => '',
                'info' => '从微信支付沙箱/仿真系统获取的 sandbox_signkey，仅在开启小程序支付沙箱模式时生效。',
                'sort' => 68,
            ],
        ];

        foreach ($items as $configKey => $item) {
            if ($this->dao->keyExists($configKey)) {
                continue;
            }

            $this->dao->create([
                'config_classify_id' => $configClassify->config_classify_id,
                'config_name' => $item['config_name'],
                'config_key' => $configKey,
                'config_type' => $item['config_type'],
                'config_rule' => $item['config_rule'],
                'config_props' => $item['config_props'],
                'required' => 0,
                'info' => $item['info'],
                'sort' => $item['sort'],
                'user_type' => 0,
    ...
    public function form($cid, ?int $id = null, array $formData = []): Form
    {
        ...
        $rule = [
            ...
            Elm::input('config_name', '配置名称：')->placeholder('请输入配置名称')->required('请输入配置名称')->appendRule('suffix', [
                'type' => 'div',
                'style' => ['color' => 'red'],
                'domProps' => [
        ...
        $rule = array_merge($rule, [
            Elm::input('config_key', '配置key：')->placeholder('请输入配置key')->required('请输入配置key'),
            ...
            Elm::select('config_type', '配置类型：')->options(function () {
                ...
                return $options;
            })->placeholder('请选择配置类型')->required('请选择配置类型'),
            ...
            Elm::switches('required', '必填：', 0)->activeValue(1)->inactiveValue(0)->inactiveText('关闭')->className('switch-width-double')->activeText('开启'),

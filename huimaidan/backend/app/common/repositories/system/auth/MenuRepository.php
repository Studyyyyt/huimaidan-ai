    ...
    public function getTree($merType = 0, $isAgent = false, $circleId = 0)
    {
        if($isAgent) {
            if($circleId) {
                ...
                $options = $this->dao->getAllOptions(2, false, $where);

                return formatTree($options, 'menu_name');
            }

            $options = $this->dao->getAllOptions(0, false, [], 'menu_name,pid', $isAgent);
            return formatTree($options, 'menu_name');
        }
    ...
    public function commandMenu($type, $data, $prompt)
    {
        ...
        foreach ($data as $key => $value) {
            try {
                ...
                if ($result && $key !== 'self') {
                    $parent = $this->dao->getMenuPid($key, $isMer, 1, $isAgent);
                    if ($parent) {
                        $pid = (int)$parent['menu_id'];
                        $path = $parent['path'] . $parent['menu_id'] . '/';
                        if ((int)$result['pid'] !== $pid || $result['path'] !== $path) {
                            $this->dao->updatePath($result['path'] . $result['menu_id'] . '/', $path . $result['menu_id'] . '/');
                            $this->dao->update((int)$result['menu_id'], [
                                'pid' => $pid,
                                'path' => $path,
                            ]);
                            $result['pid'] = $pid;
                            $result['path'] = $path;
                            printf($this->styles['info'], '修正权限父级: ' . $key);
                            echo PHP_EOL;
                        }
                    }
                }

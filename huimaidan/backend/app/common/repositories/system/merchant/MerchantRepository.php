...
{
    const HUIMAIDAN_SETTLEMENT_POOL = 1;
    const HUIMAIDAN_SETTLEMENT_WITHDRAW = 2;
    ...
    public function lst(array $where, $page, $limit)
    {
        ...
        $list = $query->page($page, $limit)
                      ...
                      ->field('sort,mer_id,mer_name,real_name,mer_phone,mer_address,mark,status,create_time,is_best,is_trader,type_id,category_id,city_id,copy_product_num,export_dump_num,is_margin,margin,ot_margin,mer_avatar,margin_remind_time,mer_banner,region_id,business_id') // 指定查询的字段
                      ->select(); // 执行查询
    ...
    public function form(?int $id = null, array $formData = [])
    {
        ...
        $rule = [
            ...
            Elm::select('city_id', '所属城市：')->options(function () {
                $options = [];
                foreach (CityArea::getDB()->where('level', 2)->order('id ASC')->column('name', 'id') as $value => $label) {
            ...
            Elm::radio('huimaidan_settlement_mode', '惠买单合作模式：', $formData['huimaidan_settlement_mode'] ?? self::HUIMAIDAN_SETTLEMENT_POOL)
                ->setOptions([
                    ['value' => self::HUIMAIDAN_SETTLEMENT_POOL, 'label' => '垫资池模式'],
                    ['value' => self::HUIMAIDAN_SETTLEMENT_WITHDRAW, 'label' => '提现模式'],
                ])->col(12),
            Elm::number('huimaidan_withdraw_rate', '惠买单提现手续费率(%)：', $formData['huimaidan_withdraw_rate'] ?? 0)->min(0)->max(100)->precision(2)->col(12),
            Elm::number('huimaidan_min_extract_money', '惠买单最低提现金额(元)：', $formData['huimaidan_min_extract_money'] ?? 500)->min(0)->precision(2)->col(12),
            // 配置销量字段，用于平台管理员手动设置展示销量
            Elm::number('configured_sales', '配置销量：', $formData['configured_sales'] ?? 0)->min(0)->precision(0)->placeholder('请输入展示基础销量'),
            // 商户关键字字段
            Elm::input('mer_keyword', '商户关键字：')->placeholder('请输入商户关键字')->col(12),
            // 商户地址字段
            Elm::input('mer_address', '商户地址：')->placeholder('请输入商户地址'),
            Elm::input('long', '经度：')->placeholder('请输入经度，例如 104.080000')->col(12),
            Elm::input('lat', '纬度：')->placeholder('请输入纬度，例如 30.650000')->col(12),
            // 微信分账商户号字段
    ...
    public function merchantForm(array $formData = [])
    {
        ...
        $rule = [
            ...
            Elm::input('branch_name', '分店名：')->placeholder('请输入分店名，例如摩尔城店'),
            Elm::number('configured_sales', '配置销量：', 0)->min(0)->precision(0)->placeholder('请输入展示基础销量'),
            Elm::number('per_capita', '人均消费：', 0)->min(0)->precision(2)->placeholder('请输入人均消费金额'),
            Elm::textarea('business_hours', '营业时间：')->placeholder("一行一条，例如：\n周一至周日 10:00-22:00"),
            Elm::switches('has_large_table', '大桌：', 0)->activeValue(1)->inactiveValue(0)->inactiveText('关闭')->className('switch-width-double')->activeText('开启')->col(12),
            Elm::switches('has_baby_chair', '宝宝椅：', 0)->activeValue(1)->inactiveValue(0)->inactiveText('关闭')->className('switch-width-double')->activeText('开启')->col(12),
            Elm::switches('can_phone_reserve', '电话预订：', 0)->activeValue(1)->inactiveValue(0)->inactiveText('关闭')->className('switch-width-double')->activeText('开启')->col(12),
            Elm::switches('is_non_smoking', '无烟区：', 0)->activeValue(1)->inactiveValue(0)->inactiveText('关闭')->className('switch-width-double')->activeText('开启')->col(12),
            Elm::frameImage('promo_image', '促销图：', '/' . config('admin.merchant_prefix') . '/setting/uploadPicture?field=promo_image&type=1')->icon('el-icon-camera')->modal(['modal' => false])->width('1000px')->height('600px')->props(['footer' => false]),
            Elm::input('slogan', '商户标语：')->placeholder('请输入商户标语'),
            // 是否开启店铺的开关按钮
    ...
    public function updateForm($id)
    {
        ...
        // 从 merchant_profile 表加载 configured_sales 数据
        /** @var \app\common\repositories\huimaidan\MerchantProfileRepository $profileRepository */
        $profileRepository = app()->make(\app\common\repositories\huimaidan\MerchantProfileRepository::class);
        $profileData = $profileRepository->formData((int)$id);
        $data['configured_sales'] = $profileData['configured_sales'] ?? 0;
    ...
    {
        $data = array_merge($data, $this->normalizeHuimaidanSettlementConfig($data));
        $data['status'] = (int)($data['status'] ?? 1);
        $data['mer_state'] = (int)($data['mer_state'] ?? 1);
        if (empty($data['city_id'])) {
            $data['city_id'] = $this->inferCityIdFromAddress((string)($data['mer_address'] ?? ''));
        }
        ...
        if (!empty($data['city_id']) && !$this->cityExists((int)$data['city_id']))
            throw new ValidateException('所属城市不存在');
    ...
    public function cityExists(int $cityId): bool
    {
        return CityArea::getDB()->where('id', $cityId)->whereIn('level', [1, 2, 3])->count() > 0;
    }

    public function inferCityIdFromAddress(string $address): int
    {
    ...
    public function normalizeHuimaidanSettlementConfig(array $data): array
    {
        $mode = (int)($data['huimaidan_settlement_mode'] ?? self::HUIMAIDAN_SETTLEMENT_POOL);
        if (!in_array($mode, [self::HUIMAIDAN_SETTLEMENT_POOL, self::HUIMAIDAN_SETTLEMENT_WITHDRAW], true)) {
            throw new ValidateException('惠买单合作模式有误');
        }

        $rate = number_format(round((float)($data['huimaidan_withdraw_rate'] ?? 0), 2), 2, '.', '');
        if (bccomp($rate, '0.00', 2) < 0 || bccomp($rate, '100.00', 2) > 0) {
            throw new ValidateException('惠买单提现手续费率必须在0到100之间');
        }

        $minExtractMoney = number_format(round((float)($data['huimaidan_min_extract_money'] ?? 500), 2), 2, '.', '');
        if (bccomp($minExtractMoney, '0.00', 2) < 0) {
            throw new ValidateException('惠买单最低提现金额不能小于0');
        }

        return [
            'huimaidan_settlement_mode' => $mode,
            'huimaidan_withdraw_rate' => $rate,
            'huimaidan_min_extract_money' => $minExtractMoney,
        ];
    }

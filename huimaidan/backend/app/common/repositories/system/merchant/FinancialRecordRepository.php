...
class FinancialRecordRepository extends BaseRepository
{
    ...
    public function getFiniancialTitle(?int $merId, array $where)
    {
         ...
         */
        $where['is_mer'] = $merId;
        // [已禁用二级分销] 原包含 brokerage_two 和 refund_brokerage_two，现移除
        if ($merId) {
            //商户收入
            $income = $this->dao->search($where)->where('financial_type', 'in', ['order', 'mer_presell'])->sum('number');
            //商户支出
            $expend_ = $this->dao->search($where)->where('financial_type', 'in', ['refund_order', 'brokerage_one', 'order_charge', 'presell_charge'])->sum('number');
            $_expend = $this->dao->search($where)->where('financial_type', 'in', ['refund_charge', 'refund_brokerage_one'])->sum('number');
            ...
            //平台支出
            $expend = $this->dao->search($where)->where('financial_type', 'in', ['brokerage_one', 'order_true', 'refund_charge', 'presell_true', 'order_platform_coupon', 'order_svip_coupon'])->sum('number');
            $msg = '平台';
        }
        $data = [
            [
    ...
    public function getAdminTitle($where)
    {
        ...
        //佣金支出金额 [已禁用二级分销] 原包含 brokerage_two 和 refund_brokerage_two，现移除
        $brokerage_ = $this->dao->search($where)->where('financial_type', 'in', ['brokerage_one'])->sum('number');
        $_brokerage = $this->dao->search($where)->where('financial_type', 'in', ['refund_brokerage_one'])->sum('number');
    ...
    public function getMerchantTitle($where)
    {
        ...
        $refund_order = $this->dao->search($where)->where('financial_type', 'refund_order')->sum('number');
        //佣金支出金额 [已禁用二级分销] 原包含 brokerage_two 和 refund_brokerage_two，现移除
        $_brokerage = $this->dao->search($where)->where('financial_type', 'in', ['brokerage_one'])->sum('number');
        $refund_brokerage = $this->dao->search($where)->where('financial_type', 'in', ['refund_brokerage_one'])->sum('number');
...
//        $bill_order = app()->make(StoreOrderRepository::class)->search(['paid' => 1,'date' => $where['date'],'pay_type' => 0])->sum('pay_price');
    ...
    public function countExpend($type, $where, $date)
    {
         ...
         * 退回收入 refund_order + （佣金 brokerage_one,brokerage_two  -  退回佣金 refund_brokerage_two,refund_brokerage_one ） + （手续费  order_charge + 预售手续费 presell_charge  - 平台退给商户的手续费 refund_charge ）
         */
        // 退回佣金 [已禁用二级分销] 原包含 brokerage_two，现移除
        $financialType = ['brokerage_one'];
        ...
        if ($where['is_mer']) { //商户的
            ...
            //退回佣金 [已禁用二级分销] 原包含 refund_brokerage_two，现移除
            $financialType = ['refund_brokerage_one'];

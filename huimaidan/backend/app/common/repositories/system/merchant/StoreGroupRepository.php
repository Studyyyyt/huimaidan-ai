...
class StoreGroupRepository extends BaseRepository
{
     ...
     * 返回格式：[{store_group_id, name, children: [{store_group_id, name}]}]
    ...
    public function getTree(): array
    {
        ...
        $list = $this->search($where)
            ->field('store_group_id, pid, name, level')
            ->order('sort DESC, store_group_id DESC')
            ->select()
            ->toArray();
        ...
        // 构建引用索引：store_group_id => &节点引用
        $index = [];
        foreach ($list as &$item) {
            $index[$item['store_group_id']] = &$item;
        }
        unset($item);
        ...
        foreach ($list as &$item) {
            if ($item['pid'] == 0) {
                // 一级分类，直接加入树
                $tree[] = &$index[$item['store_group_id']];
            } else {
                // 二级分类，挂载到父级的 children（通过引用同步）
                if (isset($index[$item['pid']])) {
                    $index[$item['pid']]['children'][] = &$index[$item['store_group_id']];
                }
            }
        }
        unset($item);

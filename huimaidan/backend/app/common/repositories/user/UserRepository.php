...
class UserRepository extends BaseRepository
{
    ...
    public function changeLabelForm($id)
    {
        ...
        return Elm::createForm(Route::buildUrl($isArray ? 'systemUserBatchChangeLabel' : 'systemUserChangeLabel', $isArray ? [] : compact('id'))->build(), [
            ...
            Elm::selectMultiple('label_id', '用户标签：', $userLabelRepository->labels($user->label_id))->options(function () use ($data) {
                $options = [];
                // 构建下拉列表的选项数组
                foreach ($data as $value => $label) {
                    $value = (string)$value;
    ...
    public function create(string $type, array $userInfo)
    {
        ...
        if (!array_key_exists('member_level', $userInfo)) {
    ...
    public function mpQrcode(User $user)
    {
        ...
        $url = app()->make(QrcodeService::class)->getRoutineQrcodePath($name, 'pages/index/index', 'spid=' . $user['uid']);
        return $this->normalizeRoutineQrcodeUrl($url);
    }

    /**
     * 修正开发环境历史附件中带 crmeb.local 的小程序二维码图片地址
     * @param string $url
     * @return string
     */
    protected function normalizeRoutineQrcodeUrl(string $url): string
    {
        $host = strtolower((string)parse_url($url, PHP_URL_HOST));
        if (!in_array($host, ['crmeb.local', 'localhost', '127.0.0.1'], true)) {
            return $url;
        }
        ...
        return HuimaidanStoreQrcodeImageService::temporaryDevelopUrl($path) . (is_string($query) && $query !== '' ? '?' . $query : '');
    }
    ...
    /**
     * 判断本次邀请绑定是否可以发放邀请积分奖励
     * @param array $config
     * @param bool $isNewUser
     * @param int $rewardCount
     * @return bool
     */
    protected function canGiveSpreadIntegralReward(array $config, bool $isNewUser, int $rewardCount): bool
    {
        if (!$isNewUser || empty($config['integral_status']) || (int)($config['integral_user_give'] ?? 0) <= 0) {
            return false;
        }

        $rewardLimit = (int)($config['integral_user_give_limit'] ?? 0);
        return $rewardLimit <= 0 || $rewardCount < $rewardLimit;
    }
    ...
    public function bindSpread(User $user, $spreadUid)
    {
        if ($spreadUid && !$user->spread_uid && $user->uid != $spreadUid && ($spread = $this->dao->get($spreadUid)) && $spread->spread_uid != $user->uid && !$spread->cancel_time) {
            ...
            Db::transaction(function () use ($spread, $spreadUid, $user, $config) {
                ...
                $userBillRepository = app()->make(UserBillRepository::class);
                $rewardCount = 0;
                if (!empty($config['integral_status']) && (int)($config['integral_user_give'] ?? 0) > 0 && $user->isNew) {
                    $rewardCount = $userBillRepository->getSearch([
                        'uid' => $spreadUid,
                        'category' => 'integral',
                        'type' => 'spread',
                        'pm' => 1,
                ...
                if ($this->canGiveSpreadIntegralReward($config, (bool)$user->isNew, (int)$rewardCount)) {
                    $integral = (int)$config['integral_user_give'];
                    $spread->integral += $integral;
                    $userBillRepository->incBill($spreadUid, 'integral', 'spread', [
                        'link_id' => $user->uid,
                        'status' => 1,
                        'title' => '邀请好友',
                        'number' => $integral,
    ...
    public function subOrder($uid, $page, $limit, array $where = [])
    {
        ...
        if (isset($where['level']) && $where['level'] !== '' && $where['level'] != -1) {
            ...
            // [已禁用二级分销] 原支持level=2查询top_uid，现禁用

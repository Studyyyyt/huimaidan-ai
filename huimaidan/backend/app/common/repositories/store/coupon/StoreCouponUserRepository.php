...
class StoreCouponUserRepository extends BaseRepository
{
    ...
    public function usableForHuimaidan(int $uid, int $merId, $amount, int $page, int $limit): array
    {
        $rows = $this->dao->validUserCoupon($uid, $merId)->toArray();
        $usable = $this->filterHuimaidanUsableCoupons($rows, $merId, $amount);
        $count = count($usable);
        $offset = max($page - 1, 0) * $limit;
        $list = array_map(function (array $row) {
            return $this->publicHuimaidanCoupon($row);
        }, array_slice($usable, $offset, $limit));
    ...
    public function usableHuimaidanCouponById(int $uid, int $merId, $amount, int $couponUserId): array
    {
        if ($couponUserId <= 0) {
            return [];
        }
        $rows = $this->dao->validUserCoupon($uid, $merId, [$couponUserId])->toArray();
        $usable = $this->filterHuimaidanUsableCoupons($rows, $merId, $amount);

        return $usable[0] ?? [];
    }

    protected function filterHuimaidanUsableCoupons(array $rows, int $merId, $amount): array
    {
        $amount = $this->money($amount);
        return array_values(array_filter($rows, function (array $row) use ($merId, $amount) {
            return $this->isHuimaidanCouponUsable($row, $merId, $amount);
        }));
    }

    protected function isHuimaidanCouponUsable(array $row, int $merId, string $amount): bool
    {
        $coupon = (array)($row['coupon'] ?? []);
        if ((int)($coupon['status'] ?? 1) !== 1 || (int)($coupon['is_del'] ?? 0) !== 0) {
            return false;
        }
        if (bccomp($amount, $this->money($row['use_min_price'] ?? 0), 2) < 0) {
            return false;
        }

        $couponType = (int)($coupon['type'] ?? -1);
        $couponMerId = (int)($row['mer_id'] ?? 0);
        if ($couponMerId === $merId && $couponType === 0) {
            return true;
        }
        if ($couponMerId === 0 && $couponType === 10) {
            return true;
        }
        if ($couponMerId === 0 && $couponType === 12) {
            $merchantIds = array_map('intval', array_column((array)($row['product'] ?? []), 'product_id'));
            return in_array($merId, $merchantIds, true);
        }
        return false;
    }

    protected function publicHuimaidanCoupon(array $row): array
    {
        $threshold = $this->money($row['use_min_price'] ?? 0);
        return [
            'id' => (int)($row['coupon_user_id'] ?? 0),
            'couponId' => (int)($row['coupon_id'] ?? 0),
            'name' => (string)($row['coupon_title'] ?? ($row['coupon']['title'] ?? '')),
            'amount' => $this->money($row['coupon_price'] ?? 0),
            'threshold' => $threshold,
            'condition' => bccomp($threshold, '0.00', 2) > 0 ? '满' . $threshold . '元可用' : '无门槛',
            'expireTime' => (string)($row['end_time'] ?? ''),
            'usedTime' => (string)($row['use_time'] ?? ''),
            'status' => $this->couponStatusText((int)($row['status'] ?? 0)),
        ];
    }

    protected function couponStatusText(int $status): string
    {
        $map = [
            0 => 'unused',
            1 => 'used',

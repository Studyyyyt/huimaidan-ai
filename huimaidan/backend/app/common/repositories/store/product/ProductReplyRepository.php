...
class ProductReplyRepository extends BaseRepository
{
    ...
    public function form(?int $productId)
    {
        ...
        // 添加用户头像图像框架选择字段，必填。
        $rule[] = Elm::frameImage('avatar', '用户头像：', '/' . config('admin.admin_prefix') . '/setting/uploadPicture?field=avatar&type=1')->width('1000px')->height('600px')->props(['footer' => false])->icon('el-icon-camera')->modal(['modal' => false])->required('请选择用户头像');

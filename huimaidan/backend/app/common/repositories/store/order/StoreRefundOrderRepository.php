...
//            throw new ValidateException('最高可退款' . floatval($totalRefundPrice) . '元');
    ...
    public function refundAfter(StoreRefundOrder $refundOrder)
    {
        ...
        // 退款后商圈代理冻结中的提成失效
        app()->make(CircleFinancialRecordRepository::class)->invalid($refundOrder->order);
    }
    ...
    private function createRefundService(int $serverType)
    {
        if ($serverType == 0) {
            ...
            return WechatService::create()->combinePay();
        } elseif ( $serverType == 2) {
            return MiniProgramService::create();
     ...
     * @param int $uid 用户ID
     * @param array $data 退款数据 [refund_message, mark, pics]
    ...
    public function orderRefund(StoreOrder $order, int $uid, array $data): StoreRefundOrder
    {
        ...
        $refundData = [
            ...
            'mark'              => $data['mark'] ?? '',
            'pics'              => $data['pics'] ?? [],
            'status'            => self::REFUND_STATUS_WAIT, // 待审核
        ...
        // 5. 事务处理
        return Db::transaction(function () use ($refundData, $products, $order, $uid) {
            // 创建退款单

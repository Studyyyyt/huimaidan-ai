...
class MerchantUpdateValidate extends Validate
{
    ...
    protected $rule = [
        ...
        'long|店铺经度' => 'checkLocation',
        'lat|店铺纬度' => 'checkLocation',
        'services_type|联系客服方式' => 'require|in:0,1',
    ];

    protected function checkLocation($value, $rule, $data = [], $field = '', $title = '')
    {
        $long = trim((string)($data['long'] ?? ''));
        ...
        if ($longitude < -180 || $longitude > 180) {
            return '经度范围为 -180 到 180';
        }

        if ($latitude < -90 || $latitude > 90) {
            return '纬度范围为 -90 到 90';
        }

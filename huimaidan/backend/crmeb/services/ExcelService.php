...
use app\common\repositories\huimaidan\PoolRepository as HuimaidanPoolRepository;
use app\common\repositories\huimaidan\SettlementRepository as HuimaidanSettlementRepository;
...
//        unset($where['status']);
    ...
    public function exportFinancial(array $where, int $page, int $limit)
    {
        ...
        */
        ...
        // [已禁用二级分销] 原包含 refund_brokerage_two，现移除
        $sys_pm_1 = ['order', 'presell', 'order_charge', 'order_presell', 'presell_charge', 'refund_brokerage_one', 'points_order_true','svip'];
        $mer_pm_1 = ['order', 'presell', 'refund_charge', 'refund_brokerage_one', 'mer_presell', 'order_platform_coupon'];
        $date_ = $where['date'];
        unset($where['date']);
        $make = app()->make(FinancialRecordRepository::class);
    ...
    public function huimaidanPoolTransactions(array $where, int $page, int $limit)
    {
        $title = ['惠买单垫资池流水', '生成时间:' . date('Y-m-d H:i:s')];
        $header = ['流水ID', '商户ID', '垫资池ID', '类型', '金额', '变动前余额', '变动后余额', '关联订单ID', '备注', '操作人ID', '创建时间'];
        $filename = '惠买单垫资池流水_' . date('YmdHis');
        $typeLabels = [1 => '平台充值', 2 => '订单扣减', 3 => '调整增加', 4 => '调整减少'];
        $data = app()->make(HuimaidanPoolRepository::class)->transactions($where, $page, $limit);
        $export = [];
    ...
    public function huimaidanOrders(array $where, int $page, int $limit)
    {
        $title = ['惠买单订单明细', '生成时间:' . date('Y-m-d H:i:s')];
        $header = ['订单ID', '订单号', '商户', '用户', '用户实付', '商家底价', '平台差价', '支付状态', '支付时间', '创建时间'];
        $filename = '惠买单订单明细_' . date('YmdHis');
        $data = app()->make(HuimaidanSettlementRepository::class)->orderList($where, $page, $limit);
        $export = [];
        foreach ($data['list'] as $item) {
            $export[] = [
                $item->order_id,
    ...
    public function huimaidanMerchants(array $where, int $page, int $limit)
    {
        $title = ['惠买单商户汇总', '生成时间:' . date('Y-m-d H:i:s')];
        $header = ['商户ID', '商户名称', '订单数量', '用户实付合计', '商家底价合计', '平台差价合计'];
        $filename = '惠买单商户汇总_' . date('YmdHis');
        $data = app()->make(HuimaidanSettlementRepository::class)->merchantStats($where, $page, $limit);
        $export = [];
        foreach ($data['list'] as $item) {
            $export[] = [
                $item->mer_id,

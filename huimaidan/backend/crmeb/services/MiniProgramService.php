...
class MiniProgramService
{
    ...
    public static function getConfig()
    {
        ...
        $payment = systemConfig([
            ...
            'pay_routine_sandbox_enabled',
            'pay_routine_sandbox_key',
        ...
        $isSandbox = !$payment['pay_routine_type'] && (int)$payment['pay_routine_sandbox_enabled'] === 1;
        $config = [
            'type' => 'routine',
            ...
            'payment' => [
                'app_id' => $wechat['routine_appId'],
                'key'    => trim($isSandbox ? $payment['pay_routine_sandbox_key'] : $payment['pay_routine_key']),
                'origin_key' => trim($payment['pay_routine_key']),
                'sandbox_sign_key' => trim($payment['pay_routine_sandbox_key']),
    ...
    {
        $merchant = $this->service->merchant;
        if ($this->isV2SandboxEnabled()) {
            $this->applySandboxSignKey($merchant);
        }

        return new PaymentService($merchant);
    }
    ...
    /**
     * 使用手机号快速验证组件返回的 code 换取手机号。
     *
     * @param string $code
     * @return array
     */
    public function getPhoneNumberByCode(string $code): array
    {
        if ($code === '') {
            throw new ValidateException('请获取手机号授权 code');
        }

        $accessToken = $this->miniProgram()->access_token->getToken();
        $url = 'https://api.weixin.qq.com/wxa/business/getuserphonenumber?access_token=' . $accessToken;
        $payload = json_encode(['code' => $code], JSON_UNESCAPED_UNICODE);
        $response = HttpService::request($url, 'post', $payload, ['Content-Type: application/json'], 10);

        if ($response === false) {
            throw new ValidateException('调用微信手机号接口失败');
        }

        $data = json_decode($response, true);
        if (!is_array($data)) {
            throw new ValidateException('微信手机号接口返回异常');
        }

        if (($data['errcode'] ?? 0) !== 0) {
            throw new ValidateException($data['errmsg'] ?? '获取手机号失败');
        }

        $phoneInfo = $data['phone_info'] ?? [];
        if (!is_array($phoneInfo) || empty($phoneInfo['purePhoneNumber']) && empty($phoneInfo['phoneNumber'])) {
            throw new ValidateException('微信手机号接口未返回手机号');
        }
    ...
    {
        if ($this->isV2SandboxEnabled()) {
            throw new ValidateException('微信支付官方仿真系统当前不支持小程序支付v2统一下单（unifiedorder/JSAPI），请切回正式环境测试，或升级到支持的支付方案');
        }
    ...
    public function query($orderNo)
    {
        if ($this->config['pay_routine_new_mchid'] ?? false) {
            throw new ValidateException('当前小程序支付配置暂不支持主动查单');
        }
        if ($this->isV3()) {
            throw new ValidateException('微信支付v3暂不支持主动查单');
        }

        return $this->paymentService()->query($orderNo);
    }

    protected function isV2SandboxEnabled(): bool
    {
        return !$this->isV3() && !empty($this->config['payment']['sandbox_mode']);
    }

    protected function applySandboxSignKey($merchant): void
    {
        if (empty($this->config['payment']['origin_key'])) {
            throw new ValidateException('请先配置小程序支付正式密钥');
        }

        $merchant->key = SandboxApi::resolveSandboxSignKey($merchant);
    }

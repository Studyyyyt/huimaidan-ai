...
Route::group(config('admin.admin_prefix'), function () {
    // AI 推荐大脑后台管理入口：跳转到独立管理页面
    Route::get('huimaidan/ai', function () {
        return redirect('/huimaidan_ai_admin.html');
    })->name('adminHuimaidanAiStandalone');

    Route::get('huimaidan/ai-admin', function () {
        return redirect('/huimaidan_ai_admin.html');
    })->name('adminHuimaidanAiAdmin');

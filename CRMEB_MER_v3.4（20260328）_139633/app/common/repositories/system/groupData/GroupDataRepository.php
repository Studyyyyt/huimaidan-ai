...
class GroupDataRepository extends BaseRepository
{
    ...
    public function reSetDataForm(int $groupId, ?int $id, ?int $merId)
    {
        ...
        $form = Elm::createForm($url);
        $rules = [
            Elm::input('svip_name', '会员名：')->required('请输入会员名'),
            ...
            Elm::number('sort', '排序：'),
            Elm::switches('status', '是否开启：')->activeValue(1)->inactiveValue(0)->inactiveText('关闭')->className('switch-width-double')->activeText('开启'),
        ];
        $form->setRule($rules);

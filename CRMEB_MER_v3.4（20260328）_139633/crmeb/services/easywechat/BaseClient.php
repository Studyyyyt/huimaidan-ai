...
//            'Wechatpay-Serial' => $this->app['config']['payment']['serial_no']
        ...
        if ($serial) {
            ...
            $options['headers']['Wechatpay-Serial'] = $this->app->certficates->setServiceStatus
            ($this->isService)->get()['serial_no'];
        }
...
//            'bank_address_code',
        ...
        if ($public_key && ($this->isService || $this->app['config']['is_v3'])) {
            ...
            if ($public_key) {
                if (!is_file($public_key))
                    throw new InvalidConfigException('公钥文件不存在');
                $public_content = file_get_contents($public_key);
                if (openssl_public_encrypt($string, $encrypted, $public_content,
    ...
    protected function processParams(array $params)
    {
        ...
        foreach ($params as $k => $v) {
            if (is_array($v)) {
                ...
                $params[$k] = $this->processParams((array)$v);
            } else {
    ...
    {
        $key_path = $this->isService ?
            $this->app['config']['service_payment']['key_path'] :
            $this->app['config']['payment']['key_path'];
        if (!file_exists($key_path) || !is_file($key_path)) {
            throw new \InvalidArgumentException("【请上传".($this->isService ? '服务商':'微信')."支付书】SSL certificate not found");
        }

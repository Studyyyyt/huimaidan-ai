...
### 1.1 交付包中为什么没有 zip 压缩包？

在最终交付前，根目录下原有的 3 个历史压缩包已被清理：

| 原 zip 文件 | 说明 | 清理原因 |
| --- | --- | --- |
| `CRMEB_MER_v3.4（20260328）_139633.zip` | 原始 CRMEB 源码压缩包 | 内容已解压到同名目录，保留 zip 会造成冗余 |
| `huimaidan.zip` | 项目中期打包的 `huimaidan/` 目录备份 | 日期早于交付清理，可能包含旧配置或已删除文件 |
| `deploy-wsl-php73-swoole4.zip` | WSL 部署脚本压缩包 | 内容已解压到同名目录，保留 zip 会造成冗余 |

如果你从其他渠道拿到还包含这 3 个 zip 的版本，建议直接使用解压后的目录，不要解压覆盖当前已清理的目录，避免引入旧配置或敏感信息。
...
3. 部署完成后，修改 `.env` 中数据库/Redis 地址为 `127.0.0.1`。
4. 手动导入 SQL。
5. 启动 Supervisor 队列。
